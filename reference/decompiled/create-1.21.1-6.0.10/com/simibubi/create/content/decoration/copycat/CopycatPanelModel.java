...
public class CopycatPanelModel extends CopycatModel {
   ...
   protected List<BakedQuad> getCroppedQuads(
      ...
      } else if (specialCopycatModelState != null
         && blockRenderer.getBlockModel((BlockState)specialCopycatModelState.setValue(DirectionalBlock.FACING, facing)) instanceof CopycatModel cm) {
         ...
         for (boolean front : Iterate.trueAndFalse) {
            ...
            for (int i = 0; i < size; i++) {
               ...
               if ((!front || direction != facing) && (front || direction != facing.getOpposite())) {
                  quads.add(
                     BakedQuadHelper.cloneWithCustomGeometry(quad, BakedModelHelper.cropAndMove(quad.getVertices(), quad.getSprite(), bb, normalScaledN13))
                  );
               }
            }
         }
