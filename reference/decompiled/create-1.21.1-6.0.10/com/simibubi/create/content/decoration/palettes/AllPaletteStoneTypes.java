...
public enum AllPaletteStoneTypes {
   ...
   ASURINE(
      PaletteBlockPattern.STANDARD_RANGE,
      r -> r.paletteStoneBlock("asurine", () -> Blocks.DEEPSLATE, true, true).properties(p -> p.destroyTime(1.25F).mapColor(MapColor.COLOR_BLUE)).register()
   ),
   CRIMSITE(
      PaletteBlockPattern.STANDARD_RANGE,
      r -> r.paletteStoneBlock("crimsite", () -> Blocks.DEEPSLATE, true, true).properties(p -> p.destroyTime(1.25F).mapColor(MapColor.COLOR_RED)).register()
   ),
   LIMESTONE(
      PaletteBlockPattern.STANDARD_RANGE,
      r -> r.paletteStoneBlock("limestone", () -> Blocks.SANDSTONE, true, false).properties(p -> p.destroyTime(1.25F).mapColor(MapColor.SAND)).register()
   ),
   OCHRUM(
      PaletteBlockPattern.STANDARD_RANGE,
      r -> r.paletteStoneBlock("ochrum", () -> Blocks.CALCITE, true, true)
            .properties(p -> p.destroyTime(1.25F).mapColor(MapColor.TERRACOTTA_YELLOW))
            .register()
   ),
   SCORIA(
      PaletteBlockPattern.STANDARD_RANGE,
      r -> r.paletteStoneBlock("scoria", () -> Blocks.BLACKSTONE, true, false).properties(p -> p.mapColor(MapColor.COLOR_BROWN)).register()
   ),
   SCORCHIA(
      PaletteBlockPattern.STANDARD_RANGE,
      r -> r.paletteStoneBlock("scorchia", () -> Blocks.BLACKSTONE, true, false).properties(p -> p.mapColor(MapColor.TERRACOTTA_GRAY)).register()
   ),
   VERIDIUM(
      PaletteBlockPattern.STANDARD_RANGE,
      r -> r.paletteStoneBlock("veridium", () -> Blocks.TUFF, true, true).properties(p -> p.destroyTime(1.25F).mapColor(MapColor.WARPED_NYLIUM)).register()
   );
