...
public class MetalScaffoldingBlockItem extends ScaffoldingBlockItem {
   ...
   public BlockPlaceContext updatePlacementContext(BlockPlaceContext pContext) {
      ...
      if (!blockstate.is(block)) {
         ...
         int i = 0;
         MutableBlockPos blockpos$mutableblockpos = blockpos.mutable().move(direction);
         ...
         while (i < 7) {
            if (!level.isClientSide && !level.isInWorldBounds(blockpos$mutableblockpos)) {
               ...
               if (player instanceof ServerPlayer sp && blockpos$mutableblockpos.getY() >= j) {
                  sp.sendSystemMessage(Component.translatable("build.tooHigh", new Object[]{j - 1}).withStyle(ChatFormatting.RED), true);
               }
               break;
            }
