...
public class SpecialCopycatPanelBlockState extends SpecialBlockStateGen {
   ...
   private Direction facing(BlockState state) {
      return (Direction)state.getValue(DirectionalBlock.FACING);
   }

   @Override
   public <T extends Block> ModelFile getModel(DataGenContext<Block, T> ctx, RegistrateBlockstateProvider prov, BlockState state) {
      BlockModelProvider models = prov.models();
      return this.facing(state).getAxis() == Axis.Y
         ? models.getExistingFile(prov.modLoc("block/copycat_panel/" + this.name + "_vertical"))
         : models.getExistingFile(prov.modLoc("block/copycat_panel/" + this.name));
   }
}
