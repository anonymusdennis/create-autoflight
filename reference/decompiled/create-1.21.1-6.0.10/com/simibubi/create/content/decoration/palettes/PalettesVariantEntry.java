...
public class PalettesVariantEntry {
   ...
   public PalettesVariantEntry(String name, AllPaletteStoneTypes paletteStoneVariants) {
      ...
      for (PaletteBlockPattern pattern : paletteStoneVariants.variantTypes) {
         BlockBuilder<? extends Block, CreateRegistrate> builder = ((BlockBuilder)REGISTRATE.block(pattern.createName(name), pattern.getBlockFactory())
               .initialProperties(baseBlock)
               .transform(TagGen.pickaxeOnly()))
            .blockstate(pattern.getBlockStateGenerator().apply(pattern).apply(name)::accept);
         ...
         pattern.createCTBehaviour(name).ifPresent(b -> builder.onRegister(CreateRegistrate.connectedTextures((Supplier<ConnectedTextureBehaviour>)b)));
         builder.recipe((c, p) -> {
      ...
      REGISTRATE.addDataGenerator(
         ProviderType.RECIPE, p -> p.stonecutting(DataIngredient.tag(paletteStoneVariants.materialTag), RecipeCategory.BUILDING_BLOCKS, baseBlock)
      );
      REGISTRATE.addDataGenerator(ProviderType.ITEM_TAGS, p -> p.addTag(paletteStoneVariants.materialTag).add(((Block)baseBlock.get()).asItem()));
