...
public class PaletteBlockPattern {
   ...
   public static final PaletteBlockPattern BRICKS = create("cut_bricks", PaletteBlockPattern.PatternNameType.WRAP, PaletteBlockPartial.ALL_PARTIALS)
      .textures("brick");
   public static final PaletteBlockPattern SMALL_BRICKS = create("small_bricks", PaletteBlockPattern.PatternNameType.WRAP, PaletteBlockPartial.ALL_PARTIALS)
      .textures("small_brick");
   public static final PaletteBlockPattern POLISHED = create("polished_cut", PaletteBlockPattern.PatternNameType.PREFIX, PaletteBlockPartial.FOR_POLISHED)
      .textures("polished", "slab");
   public static final PaletteBlockPattern LAYERED = create("layered", PaletteBlockPattern.PatternNameType.PREFIX)
   ...
   public static final PaletteBlockPattern PILLAR = create("pillar", PaletteBlockPattern.PatternNameType.SUFFIX)
   ...
   public PaletteBlockPattern.IBlockStateProvider pillar(String variant) {
      ...
      return (ctx, prov) -> prov.getVariantBuilder((Block)ctx.getEntry())
            .forAllStatesExcept(
               state -> {
                  Axis axis = (Axis)state.getValue(BlockStateProperties.AXIS);
                  return axis == Axis.Y
                     ? ConfiguredModel.builder().modelFile(prov.models().cubeColumn(this.createName(variant), side, end)).uvLock(false).build()
                     : ConfiguredModel.builder()
                        .modelFile(prov.models().cubeColumnHorizontal(this.createName(variant) + "_horizontal", side, end))
                        .uvLock(false)
                        .rotationX(90)
   ...
   protected String createName(String variant) {
      ...
      return this.nameType == PaletteBlockPattern.PatternNameType.SUFFIX
         ? String.format(formatString, variant, this.id)
         : String.format(formatString, this.id, variant);
   }
   ...
   protected static CTSpriteShiftEntry ct(String variant, PaletteBlockPattern.CTs texture) {
      ...
      return CTSpriteShifter.getCT(
         texture.type, resLoc, ResourceLocation.fromNamespaceAndPath(resLocTarget.getNamespace(), resLocTarget.getPath() + "_connected")
      );
   }
