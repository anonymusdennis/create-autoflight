...
public class MetalScaffoldingCTBehaviour extends HorizontalCTBehaviour {
   ...
   public CTSpriteShiftEntry getShift(BlockState state, Direction direction, @Nullable TextureAtlasSprite sprite) {
      return direction.getAxis() != Axis.Y && sprite == this.insideShift.getOriginal() ? this.insideShift : super.getShift(state, direction, sprite);
   }

   @Override
   public boolean connectsTo(BlockState state, BlockState other, BlockAndTintGetter reader, BlockPos pos, BlockPos otherPos, Direction face) {
      return super.connectsTo(state, other, reader, pos, otherPos, face)
         && (Boolean)state.getValue(MetalScaffoldingBlock.BOTTOM)
         && (Boolean)other.getValue(MetalScaffoldingBlock.BOTTOM);
   }
}
