...
public abstract class PaletteBlockPartial<B extends Block> {
   ...
   @NonnullType
   public BlockBuilder<B, CreateRegistrate> create(
      String variantName, PaletteBlockPattern pattern, BlockEntry<? extends Block> block, AllPaletteStoneTypes variant
   ) {
      String patternName = Lang.nonPluralId(pattern.createName(variantName));
      String blockName = patternName + "_" + this.name;
      BlockBuilder<B, CreateRegistrate> blockBuilder = (BlockBuilder<B, CreateRegistrate>)Create.registrate()
         .block(blockName, p -> this.createBlock(block))
         .blockstate((c, p) -> this.generateBlockState(c, p, variantName, pattern, block))
         .recipe((c, p) -> this.createRecipes(variant, block, c, p))
         .transform(b -> this.transformBlock(b, variantName, pattern));
      ItemBuilder<BlockItem, BlockBuilder<B, CreateRegistrate>> itemBuilder = (ItemBuilder<BlockItem, BlockBuilder<B, CreateRegistrate>>)blockBuilder.item()
         .transform(b -> this.transformItem(b, variantName, pattern));
      if (this.canRecycle()) {
   ...
   protected ItemBuilder<BlockItem, BlockBuilder<B, CreateRegistrate>> transformItem(
      ItemBuilder<BlockItem, BlockBuilder<B, CreateRegistrate>> builder, String variantName, PaletteBlockPattern pattern
   ) {
   ...
   protected abstract void createRecipes(
      AllPaletteStoneTypes var1, BlockEntry<? extends Block> var2, DataGenContext<Block, ? extends Block> var3, RegistrateRecipeProvider var4
   );

   protected abstract void generateBlockState(
      DataGenContext<Block, B> var1, RegistrateBlockstateProvider var2, String var3, PaletteBlockPattern var4, Supplier<? extends Block> var5
   );
   ...
   private static class Slab extends PaletteBlockPartial<SlabBlock> {
      ...
      protected SlabBlock createBlock(Supplier<? extends Block> block) {
         return new SlabBlock(Properties.ofFullCopy((BlockBehaviour)block.get()));
      }
      ...
      protected void createRecipes(
         AllPaletteStoneTypes type, BlockEntry<? extends Block> patternBlock, DataGenContext<Block, ? extends Block> c, RegistrateRecipeProvider p
      ) {
         ...
         ShapelessRecipeBuilder.shapeless(category, (ItemLike)patternBlock.get())
            .requires(ingredient.toVanilla())
            .requires(ingredient.toVanilla())
            .unlockedBy("has_" + c.getName(), ingredient.getCriterion(p))
            .save(p, "create:" + c.getName() + "_recycling");
      }
   ...
   private static class Stairs extends PaletteBlockPartial<StairBlock> {
      ...
      protected StairBlock createBlock(Supplier<? extends Block> block) {
         return new StairBlock(block.get().defaultBlockState(), Properties.ofFullCopy((BlockBehaviour)block.get()));
      }
   ...
   private static class Wall extends PaletteBlockPartial<WallBlock> {
      ...
      protected WallBlock createBlock(Supplier<? extends Block> block) {
         return new WallBlock(Properties.ofFullCopy((BlockBehaviour)block.get()).forceSolidOn());
      }

      @Override
      protected ItemBuilder<BlockItem, BlockBuilder<WallBlock, CreateRegistrate>> transformItem(
         ItemBuilder<BlockItem, BlockBuilder<WallBlock, CreateRegistrate>> builder, String variantName, PaletteBlockPattern pattern
      ) {
