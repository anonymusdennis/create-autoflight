...
public class BracketGenerator extends DirectionalAxisBlockStateGen {
   ...
   public <T extends Block> ModelFile getModel(DataGenContext<Block, T> ctx, RegistrateBlockstateProvider prov, BlockState state) {
      String type = ((BracketBlock.BracketType)state.getValue(BracketBlock.TYPE)).getSerializedName();
      boolean vertical = ((Direction)state.getValue(BracketBlock.FACING)).getAxis().isVertical();
      String path = "block/bracket/" + type + "/" + (vertical ? "ground" : "wall");
      return ((BlockModelBuilder)((BlockModelBuilder)prov.models().withExistingParent(path + "_" + this.material, prov.modLoc(path)))
            .texture("bracket", prov.modLoc("block/bracket_" + this.material)))
         .texture("plate", prov.modLoc("block/bracket_plate_" + this.material));
   }

   public static <I extends BlockItem, P> NonNullFunction<ItemBuilder<I, P>, P> itemModel(String material) {
      return b -> b.model(
               (c, p) -> ((ItemModelBuilder)((ItemModelBuilder)p.withExistingParent(c.getName(), p.modLoc("block/bracket/item")))
