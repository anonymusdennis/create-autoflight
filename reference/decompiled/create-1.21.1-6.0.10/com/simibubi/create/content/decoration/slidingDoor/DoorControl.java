...
public enum DoorControl {
   ...
   private static String[] valuesAsString() {
      ...
      return Arrays.stream(values).map(dc -> Lang.asId(dc.name())).toList().toArray(new String[values.length]);
   }
      ...
      ScrollInput input = new SelectionScrollInput(x, y, 53, 16)
         ...
         .addHint(
            CreateLang.translateDirect(
               "contraption.door_control.player_facing", CreateLang.translateDirect("contraption.door_control." + Lang.asId(playerFacing.name()) + ".short")
            )
         )
