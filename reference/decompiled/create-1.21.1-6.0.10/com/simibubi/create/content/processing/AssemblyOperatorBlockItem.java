...
public class AssemblyOperatorBlockItem extends BlockItem {
   ...
   public InteractionResult place(BlockPlaceContext context) {
      ...
      if (this.operatesOn(level, placedOnPos, placedOnState) && context.getClickedFace() == Direction.UP) {
   ...
   protected BlockPlaceContext adjustContext(BlockPlaceContext context, BlockPos placedOnPos) {
      ...
      return new AssemblyOperatorUseContext(
         ...
         new BlockHitResult(
            new Vec3(
               (double)up.getX() + 0.5 + (double)Direction.UP.getStepX() * 0.5,
               (double)up.getY() + 0.5 + (double)Direction.UP.getStepY() * 0.5,
               (double)up.getZ() + 0.5 + (double)Direction.UP.getStepZ() * 0.5
            ),
            Direction.UP,
            up,
            false
   ...
   protected boolean operatesOn(LevelReader world, BlockPos pos, BlockState placedOnState) {
      return AllBlocks.BELT.has(placedOnState)
         ? placedOnState.getValue(BeltBlock.SLOPE) == BeltSlope.HORIZONTAL
         : BasinBlock.isBasin(world, pos) || AllBlocks.DEPOT.has(placedOnState) || AllBlocks.WEIGHTED_EJECTOR.has(placedOnState);
   }
}
