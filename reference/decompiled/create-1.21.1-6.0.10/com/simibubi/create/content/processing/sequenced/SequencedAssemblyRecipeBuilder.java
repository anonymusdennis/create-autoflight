...
public class SequencedAssemblyRecipeBuilder {
   ...
   public <R extends StandardProcessingRecipe<?>> SequencedAssemblyRecipeBuilder addStep(
      StandardProcessingRecipe.Factory<R> factory, UnaryOperator<StandardProcessingRecipe.Builder<R>> builder
   ) {
      return this.addStep(id -> new StandardProcessingRecipe.Builder<>(factory, id), builder);
   }

   public <R extends ItemApplicationRecipe> SequencedAssemblyRecipeBuilder addStep(
      ItemApplicationRecipe.Factory<R> factory, UnaryOperator<ItemApplicationRecipe.Builder<R>> builder
   ) {
      return this.addStep(id -> new ItemApplicationRecipe.Builder<>(factory, id), builder);
   }
   ...
   public <B extends ProcessingRecipeBuilder<?, ?, B>> SequencedAssemblyRecipeBuilder addStep(Function<ResourceLocation, B> factory, UnaryOperator<B> builder) {
      B recipeBuilder = (B)factory.apply(ResourceLocation.withDefaultNamespace("dummy"));
      Item placeHolder = this.recipe.getTransitionalItem().getItem();
      this.recipe.getSequence().add(new SequencedRecipe<>(builder.apply(recipeBuilder.require(placeHolder).output(placeHolder)).build()));
      return this;
   }

   public SequencedAssemblyRecipeBuilder require(ItemLike ingredient) {
      return this.require(Ingredient.of(new ItemLike[]{ingredient}));
   }
   ...
   public void build(RecipeOutput consumer) {
      ...
      ResourceLocation id = ResourceLocation.fromNamespaceAndPath(
         holder.id().getNamespace(), AllRecipeTypes.SEQUENCED_ASSEMBLY.getId().getPath() + "/" + holder.id().getPath()
      );
