...
public class BlazeBurnerMovementBehaviour implements MovementBehaviour {
   ...
   public void tick(MovementContext context) {
      if (context.world.isClientSide()) {
         if (this.shouldRender(context)) {
            ...
            headAngle.chase(
               (double)(headAngle.getValue() + AngleHelper.getShortestAngleDiff((double)headAngle.getValue(), (double)this.getTargetAngle(context))),
               0.5,
               quickTurn ? Chaser.EXP : Chaser.exp(5.0)
            );
   ...
   private float getTargetAngle(MovementContext context) {
      if (this.shouldRenderHat(context)
         && !Mth.equal(context.relativeMotion.length(), 0.0)
         ...
         return cce.getInitialOrientation().getAxis() == Axis.X ? angle + 180.0F : angle;
      } else {
