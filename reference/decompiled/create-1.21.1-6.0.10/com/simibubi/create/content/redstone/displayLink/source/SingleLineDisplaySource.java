...
public abstract class SingleLineDisplaySource extends DisplaySource {
   ...
   protected void addLabelingTextBox(ModularGuiLineBuilder builder) {
      builder.addTextInput(
         ...
         (e, t) -> {
            ...
            t.withTooltip(
               ImmutableList.of(
                  CreateLang.translateDirect("display_source.label").withStyle(s -> s.withColor(5476833)),
   ...
   public List<List<MutableComponent>> provideFlapDisplayText(DisplayLinkContext context, DisplayTargetStats stats) {
      if (this.allowsLabeling(context)) {
         String label = context.sourceConfig().getString("Label");
         if (!label.isEmpty()) {
            return ImmutableList.of(ImmutableList.of(Component.literal(label + " "), this.provideLine(context, stats)));
         }
      }
