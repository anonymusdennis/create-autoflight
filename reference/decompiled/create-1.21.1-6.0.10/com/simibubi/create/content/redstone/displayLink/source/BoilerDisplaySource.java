...
public class BoilerDisplaySource extends DisplaySource {
   public static final List<MutableComponent> notEnoughSpaceSingle = List.of(
      CreateLang.translateDirect("display_source.boiler.not_enough_space").append(CreateLang.translateDirect("display_source.boiler.for_boiler_status"))
   );
   public static final List<MutableComponent> notEnoughSpaceDouble = List.of(
      CreateLang.translateDirect("display_source.boiler.not_enough_space"), CreateLang.translateDirect("display_source.boiler.for_boiler_status")
   );
   ...
   private Stream<List<MutableComponent>> getComponents(DisplayLinkContext context, boolean forFlapDisplay) {
      if (context.getSourceBlockEntity() instanceof FluidTankBlockEntity tankBlockEntity) {
         ...
         if (var12 == null) {
            ...
            if (forFlapDisplay) {
               size = Component.literal(Strings.repeat(' ', lw - this.labelWidthOf("size"))).append(size);
               water = Component.literal(Strings.repeat(' ', lw - this.labelWidthOf("water"))).append(water);
               heat = Component.literal(Strings.repeat(' ', lw - this.labelWidthOf("heat"))).append(heat);
            }
   ...
   private MutableComponent labelOf(String label) {
      return label.isBlank() ? Component.empty() : CreateLang.translateDirect("boiler." + label);
   }
