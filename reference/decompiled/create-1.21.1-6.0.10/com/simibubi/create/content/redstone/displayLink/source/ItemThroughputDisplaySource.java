...
public class ItemThroughputDisplaySource extends AccumulatedItemCountDisplaySource {
   ...
   public void itemReceived(DisplayLinkBlockEntity be, int amount) {
      if (!be.getBlockState().getOptionalValue(DisplayLinkBlock.POWERED).orElse(true)) {
         ...
         if (!conf.contains("LastReceived")) {
            ...
            rates.set(poolIndex, FloatTag.valueOf((float)((double)amount / (double)(gameTime - previousTime))));
            float rate = 0.0F;
   ...
   public void initConfigurationWidgets(DisplayLinkContext context, ModularGuiLineBuilder builder, boolean isFirstLine) {
      ...
      if (!isFirstLine) {
         builder.addSelectionScrollInput(
            0,
            80,
            (si, l) -> si.forOptions(CreateLang.translatedOptions("display_source.item_throughput.interval", "second", "minute", "hour"))
