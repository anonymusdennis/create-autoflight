...
public class ToggleLatchGenerator extends AbstractDiodeGenerator {
   ...
   protected <T extends Block> List<ModelFile> createModels(DataGenContext<Block, T> ctx, BlockModelProvider prov) {
      ...
      models.add(((BlockModelBuilder)prov.withExistingParent(name + "_off_powered", off)).texture("top", this.texture(ctx, "powered")));
      models.add(prov.getExistingFile(on));
      models.add(((BlockModelBuilder)prov.withExistingParent(name + "_on_powered", on)).texture("top", this.texture(ctx, "powered_powering")));
      return models;
   }
