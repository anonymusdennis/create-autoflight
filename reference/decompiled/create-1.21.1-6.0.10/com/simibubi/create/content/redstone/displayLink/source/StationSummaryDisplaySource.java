         ...
         .forEach(
            prediction -> {
               ...
               if (prediction.ticks == -1 || prediction.ticks >= 11700) {
                  ...
                  lines.add(
                     min > 0
                        ? CreateLang.translateDirect("display_source.station_summary.minutes")
   ...
   public void loadFlapDisplayLayout(DisplayLinkContext context, FlapDisplayBlockEntity flapDisplay, FlapDisplayLayout layout) {
      ...
      if (!layout.isLayout(layoutName)) {
         ...
         FlapDisplaySection destination = new FlapDisplaySection(
            (float)destinationWidth * 7.0F, "alphabet", false, hasPlatform && destinationWidth > 0 && platformWidth > 0.0F
         );
      ...
      if (isFirstLine) {
         builder.addTextInput(
            ...
            (e, t) -> {
               ...
               t.withTooltip(
                  ImmutableList.of(
                     CreateLang.translateDirect("display_source.station_summary.filter").withStyle(s -> s.withColor(5476833)),
