      ...
      if (context.getSourceBlockEntity() instanceof SmartObserverBlockEntity cobe) {
         ...
         if (handler == null) {
            ...
            return fluids.entrySet()
               .stream()
               .sorted(Comparator.<Entry<Fluid, Integer>>comparingInt(value -> value.getValue()).reversed())
               .limit((long)maxRows)
               .map(entry -> IntAttached.with(entry.getValue(), fluidNames.get(entry.getKey()).getHoverName().copy()));
         }
      } else {
         return Stream.empty();
      }
