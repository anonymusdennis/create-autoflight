...
public class LecternDisplayTarget extends DisplayTarget {
      ...
      if (context.getTargetBlockEntity() instanceof LecternBlockEntity lectern) {
         ...
         if (!book.isEmpty()) {
            ...
            if (book.is(Items.WRITTEN_BOOK)) {
               WrittenBookContent writtenBookContent = (WrittenBookContent)book.getOrDefault(DataComponents.WRITTEN_BOOK_CONTENT, WrittenBookContent.EMPTY);
               List<Filterable<Component>> pages = new ArrayList<>(writtenBookContent.pages());
               boolean changed = false;

               for (int i = 0; i - line < text.size() && i < 50; i++) {
                  if (pages.size() <= i) {
                     pages.add(Filterable.passThrough(i < line ? Component.empty() : (Component)text.get(i - line)));
                  } else if (i >= line) {
                     if (i - line == 0) {
                     ...
                     pages.set(i, Filterable.passThrough((Component)text.get(i - line)));
                  }
   ...
   private ItemStack signBook(ItemStack book) {
      ...
      List<Filterable<Component>> list = bookContents.pages().stream().map(filterable -> filterable.map(Component::literal)).toList();
