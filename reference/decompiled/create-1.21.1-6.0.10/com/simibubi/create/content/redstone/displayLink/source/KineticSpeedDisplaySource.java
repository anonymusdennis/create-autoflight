...
public class KineticSpeedDisplaySource extends NumericSingleLineDisplaySource {
   ...
   protected MutableComponent provideLine(DisplayLinkContext context, DisplayTargetStats stats) {
      if (context.getSourceBlockEntity() instanceof SpeedGaugeBlockEntity speedGauge) {
         ...
         return CreateLang.number((double)speed).space().translate("generic.unit.rpm", new Object[0]).component();
      } else {
         return ZERO.copy();
      }
   }
      ...
      if (!isFirstLine) {
         builder.addSelectionScrollInput(
            ...
            (selectionScrollInput, label) -> selectionScrollInput.forOptions(
                  CreateLang.translatedOptions("display_source.kinetic_speed", "absolute", "directional")
               ),
            "Directional"
         );
      }
