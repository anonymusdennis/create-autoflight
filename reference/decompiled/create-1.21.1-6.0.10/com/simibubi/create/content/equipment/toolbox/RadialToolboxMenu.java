...
public class RadialToolboxMenu extends AbstractSimiScreen {
   ...
   protected void renderWindow(GuiGraphics graphics, int mouseX, int mouseY, float partialTicks) {
      ...
      if (this.state == RadialToolboxMenu.State.DETACH) {
         ...
         GuiGameElement.of(AllBlocks.TOOLBOXES.get(DyeColor.BROWN).asStack()).at(-9.0F, -9.0F).render(graphics);
         ...
         if (!this.scrollMode && this.hoveredSlot == -7) {
            AllGuiTextures.TOOLBELT_SLOT_HIGHLIGHT.render(graphics, -13, -13);
            tip = CreateLang.translateDirect(this.state == RadialToolboxMenu.State.SELECT_BOX ? "toolbox.depositAll" : "toolbox.depositBox")
         ...
         for (int slot = 0; slot < 8; slot++) {
            ms.pushPose();
            ((PoseTransformStack)TransformStack.of(ms).rotateZDegrees((float)(slot * 45 - 45)))
               .translate(0.0F, -40.0F + 10.0F * (1.0F - fade) * (1.0F - fade), 0.0F)
               .rotateZDegrees((float)(-slot * 45 + 45));
            ...
            if (this.state == RadialToolboxMenu.State.SELECT_ITEM || this.state == RadialToolboxMenu.State.SELECT_ITEM_UNEQUIP) {
               ...
               if (!stackInSlot.isEmpty()) {
                  ...
                  GuiGameElement.of(stackInSlot).at(3.0F, 3.0F).render(graphics);
               ...
               if (slot < this.toolboxes.size()) {
                  ...
                  GuiGameElement.of(AllBlocks.TOOLBOXES.get(toolboxBlockEntity.getColor()).asStack()).at(3.0F, 3.0F).render(graphics);
      ...
      if (tip != null) {
         ...
         if (i1 > 8) {
            ...
            int l = this.font.width(tip);
            graphics.drawString(this.font, tip, Math.round((float)(-l) / 2.0F), -4, k1 | k, false);
      ...
      if (selected == -7) {
         if (this.state != RadialToolboxMenu.State.DETACH) {
            if (this.state == RadialToolboxMenu.State.SELECT_BOX) {
               this.toolboxes.forEach(be -> CatnipServices.NETWORK.sendToServer(new ToolboxDisposeAllPacket(be.getBlockPos())));
            } else {
               CatnipServices.NETWORK.sendToServer(new ToolboxDisposeAllPacket(this.selectedBox.getBlockPos()));
            }
         }
      } else if (this.state != RadialToolboxMenu.State.SELECT_BOX) {
         if (this.state == RadialToolboxMenu.State.DETACH) {
            if (selected == -5) {
               CatnipServices.NETWORK.sendToServer(new ToolboxEquipPacket(null, selected, this.minecraft.player.getInventory().selected));
            }
         } else {
            if (selected == -5) {
               CatnipServices.NETWORK
                  .sendToServer(new ToolboxEquipPacket(this.selectedBox.getBlockPos(), selected, this.minecraft.player.getInventory().selected));
            }
   ...
   public boolean mouseScrolled(double pMouseX, double pMouseY, double pScrollX, double pScrollY) {
      ...
      double hoveredY = pMouseY - (double)(window.getGuiScaledHeight() / 2);
      ...
      if (!(distance <= 150.0)) {
         ...
         for (int i = 0; i < 10; i++) {
            ...
            this.scrollSlot = this.scrollSlot - Mth.sign(pScrollY);
   ...
   public boolean mouseClicked(double x, double y, int button) {
      ...
      if (button == 0) {
         ...
         if ((
               this.state == RadialToolboxMenu.State.DETACH
                  || this.state == RadialToolboxMenu.State.SELECT_ITEM
                  || this.state == RadialToolboxMenu.State.SELECT_ITEM_UNEQUIP
            )
