...
public class CylinderBrush extends ShapedBrush {
   ...
   public CylinderBrush() {
      ...
      for (int i = 0; i <= 8; i++) {
         ...
         List<BlockPos> positions = BlockPos.betweenClosedStream(BlockPos.ZERO.offset(-i - 1, 0, -i - 1), BlockPos.ZERO.offset(i + 1, 0, i + 1))
            ...
            .filter(px -> VecHelper.getCenterOf(px).distanceTo(VecHelper.getCenterOf(BlockPos.ZERO)) < (double)((float)radius + 0.42F))
   ...
   Component getParamLabel(int paramIndex) {
      return (Component)(paramIndex == 0 ? CreateLang.translateDirect("generic.radius") : super.getParamLabel(paramIndex));
   }

   @Override
   public List<BlockPos> getIncludedPositions() {
      return this.cachedBrushes.get(Pair.of(this.param0, this.param1));
   }
}
