...
public class SoulParticle extends CustomRotationParticle {
   ...
   public void tick() {
      ...
      if (!SoulPulseEffect.isDark(this.level, pos)) {
         this.isVisible = true;
         if (!this.isPerimeter) {
            this.remove();
         }
   ...
   public static class PerimeterAnimation extends SoulParticle.AnimationStage {
      ...
      public SoulParticle.AnimationStage getNext() {
         return this.animAge
               < (
                  this.particle.isExpandingPerimeter
                     ? 8
                     : this.particle.startTicks + this.particle.endTicks + this.particle.numLoops * this.particle.loopLength
               )
            ? this
            : null;
      }
