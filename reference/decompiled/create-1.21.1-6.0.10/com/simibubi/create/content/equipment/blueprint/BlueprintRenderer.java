...
public class BlueprintRenderer extends EntityRenderer<BlueprintEntity> {
   ...
   public void render(BlueprintEntity entity, float yaw, float pt, PoseStack ms, MultiBufferSource buffer, int light) {
      ...
      int bl = light >> 4 & 15;
      int sl = light >> 20 & 15;
      boolean vertical = entity.getXRot() != 0.0F;
      ...
      ((PoseTransformStack)TransformStack.of(ms).rotateYDegrees(vertical ? 0.0F : -yaw)).rotateXDegrees(fakeNormalXRotation);
      Matrix3f copy = new Matrix3f(ms.last().normal());
      ms.popPose();
      ms.pushPose();
      ((PoseTransformStack)((PoseTransformStack)TransformStack.of(ms).rotateYDegrees(-yaw)).rotateXDegrees(entity.getXRot())).translate(0.0, 0.0, 0.03225);
      if (entity.size == 3) {
      ...
      for (int x = 0; x < entity.size; x++) {
         ...
         for (int y = 0; y < entity.size; y++) {
            ...
            displayItems.forEachWithContext(
               (stack, primary) -> {
                  if (!stack.isEmpty()) {
                     squashedMS.pushPose();
                     if (!primary) {
                     ...
                     Minecraft.getInstance()
                        .getItemRenderer()
                        .renderStatic(stack, ItemDisplayContext.GUI, itemLight, OverlayTexture.NO_OVERLAY, squashedMS, buffer, entity.level(), 0);
                     squashedMS.popPose();
                  }
               }
            );
