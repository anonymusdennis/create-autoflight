...
public class WorldshaperItem extends ZapperItem {
   ...
   public Component validateUsage(ItemStack item) {
      return (Component)(!item.has(AllDataComponents.SHAPER_BRUSH_PARAMS)
         ? CreateLang.translateDirect("terrainzapper.shiftRightClickToSet")
         : super.validateUsage(item));
   }

   @Override
   protected boolean canActivateWithoutSelectedBlock(ItemStack stack) {
      TerrainTools tool = (TerrainTools)stack.getOrDefault(AllDataComponents.SHAPER_TOOL, TerrainTools.Fill);
   ...
   protected boolean activate(Level level, Player player, ItemStack stack, BlockState stateToUse, BlockHitResult raytrace, CompoundTag data) {
      ...
      Brush brush = ((TerrainBrushes)stack.getOrDefault(AllDataComponents.SHAPER_BRUSH, TerrainBrushes.Cuboid)).get();
      BlockPos params = (BlockPos)stack.get(AllDataComponents.SHAPER_BRUSH_PARAMS);
      PlacementOptions option = (PlacementOptions)stack.getOrDefault(AllDataComponents.SHAPER_PLACEMENT_OPTIONS, PlacementOptions.Merged);
      TerrainTools tool = (TerrainTools)stack.getOrDefault(AllDataComponents.SHAPER_TOOL, TerrainTools.Fill);
