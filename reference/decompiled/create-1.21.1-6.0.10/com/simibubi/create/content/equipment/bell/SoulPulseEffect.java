...
public class SoulPulseEffect {
   ...
   public List<BlockPos> getPotentialSoulSpawns(Level world) {
      return (List<BlockPos>)(world == null
         ? new ArrayList<>()
         : getLayer(this.currentLayerIdx()).map(p -> p.offset(this.pos)).filter(p -> canSpawnSoulAt(world, p, true)).collect(Collectors.toList()));
   }
      ...
      return world != null
         ...
         && Streams.stream(
               world.getBlockCollisions(
                  null,
                  new AABB(
                     (double)at.getX() + 0.5 - w2,
                     (double)at.getY(),
                     (double)at.getZ() + 0.5 - w2,
                     (double)at.getX() + 0.5 + w2,
                     (double)at.getY() + dummyHeight,
                     (double)at.getZ() + 0.5 + w2
                  )
               )
            )
      ...
      if (world != null && world.isClientSide) {
         ...
         if (this.canOverlap()) {
            world.addAlwaysVisibleParticle(
               (ParticleOptions)((int)Math.round(VecHelper.getCenterOf(this.pos).distanceTo(VecHelper.getCenterOf(at))) >= this.distance
                  ? new SoulParticle.PerimeterData()
                  : new SoulParticle.ExpandingPerimeterData()),
               p.x + 0.5,
               p.y + 0.5,
               p.z + 0.5,
               0.0,
   ...
   public static Stream<BlockPos> getLayer(int idx) {
      return idx >= 0 && idx < 11 ? LAYERS.get(idx).stream() : Stream.empty();
   }
}
