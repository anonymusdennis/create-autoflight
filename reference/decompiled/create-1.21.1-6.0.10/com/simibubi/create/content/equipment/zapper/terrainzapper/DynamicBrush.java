...
public class DynamicBrush extends Brush {
   ...
   public Collection<BlockPos> addToGlobalPositions(
      LevelAccessor world, BlockPos targetPos, Direction targetFace, Collection<BlockPos> affectedPositions, TerrainTools usedTool
   ) {
      ...
      while (!frontier.isEmpty()) {
         ...
         if (!visited.contains(currentPos)) {
            ...
            if (currentPos.closerThan(startPos, (double)searchRange)) {
               if (replace) {
                  ...
                  if (stateToReplace.getDestroySpeed(world, currentPos) != -1.0F
                     && (stateToReplace.getBlock() == state.getBlock() || fuzzy)
                     && !stateToReplace.canBeReplaced()
                     && (
                        !BlockHelper.hasBlockSolidSide(stateAboveStateToReplace, world, currentPos.relative(targetFace), targetFace.getOpposite())
                           || !this.surface
                     )) {
                  ...
                  if (!stateToPlaceOn.canBeReplaced() && (stateToPlaceOn.getBlock() == state.getBlock() || fuzzy) && stateToPlaceAt.canBeReplaced()) {
                     affectedPositions.add(currentPos);
