...
public class TrackEdge {
   ...
   public double getLength() {
      return this.isInterDimensional()
         ? 0.0
         : (this.isTurn() ? this.turn.getLength() : this.node1.location.getLocation().distanceTo(this.node2.location.getLocation()));
   }
   ...
   public Collection<double[]> getIntersection(TrackNode node1, TrackNode node2, TrackEdge other, TrackNode other1, TrackNode other2) {
      ...
      if (!this.isInterDimensional() && !other.isInterDimensional()) {
         if (v1.y == v2.y && v1.y == w1.y && v1.y == w2.y) {
            if (!this.isTurn()) {
               return (Collection<double[]>)(!other.isTurn()
                  ? ImmutableList.of(VecHelper.intersectRanged(v1, w1, v2, w2, Axis.Y))
                  : other.getIntersection(other1, other2, this, node1, node2).stream().map(a -> new double[]{a[1], a[0]}).toList());
            } else {
   ...
   public static TrackEdge read(TrackNode node1, TrackNode node2, CompoundTag tag, TrackGraph graph, DimensionPalette dimensions) {
      TrackEdge trackEdge = new TrackEdge(
         node1, node2, tag.contains("Positions") ? new BezierConnection(tag, BlockPos.ZERO) : null, TrackMaterial.deserialize(tag.getString("Material"))
      );
