...
public class CarriageSyncData {
   public List<Pair<Couple<Integer>, Float>> wheelLocations = new ArrayList<>(4);
   ...
   public void write(FriendlyByteBuf buffer) {
      ...
      if (this.fallbackLocations != null) {
         ...
         Couple<Vec3> rotationAnchors = (Couple<Vec3>)this.fallbackLocations.getSecond();
         ...
         for (Pair<Couple<Integer>, Float> pair : this.wheelLocations) {
            ...
            ((Couple)pair.getFirst()).forEach(buffer::writeInt);
            buffer.writeFloat((Float)pair.getSecond());
         }
      ...
      if (this.fallbackLocations != null) {
         ...
         if (graph != null) {
            for (int i = 0; i < this.wheelLocations.size(); i++) {
               Pair<Couple<Integer>, Float> pair = this.wheelLocations.get(i);
               if (pair == null) {
                  break;
               }
               ...
               if (!nodes.either(Objects::isNull)) {
                  ...
                  if (edge != null) {
                     ...
                     point.edge = edge;
                     point.position = (double)((Float)pair.getSecond()).floatValue();
   ...
   public void approach(CarriageContraptionEntity entity, Carriage carriage, float partialIn) {
      ...
      if (this.fallbackLocations != null && this.fallbackPointSnapshot != null) {
         dce.positionAnchor = this.approachVector(
            partialIn, dce.positionAnchor, (Vec3)this.fallbackLocations.getFirst(), (Vec3)this.fallbackPointSnapshot.getFirst()
         );
         dce.rotationAnchors
            .replaceWithContext(
               (current, firstx) -> this.approachVector(
                     partialIn,
                     current,
                     (Vec3)((Couple)this.fallbackLocations.getSecond()).get(firstx),
                     (Vec3)((Couple)this.fallbackPointSnapshot.getSecond()).get(firstx)
                  )
            );
      } else {
         TrackGraph graph = carriage.train.graph;
         if (graph != null) {
            carriage.train.navigation.distanceToDestination = carriage.train.navigation.distanceToDestination
               + (double)(partialIn * this.destinationDistanceSnapshot);
   ...
   public float getDistanceTo(TrackGraph graph, TravellingPoint current, TravellingPoint target, float maxDistance, boolean forward) {
      ...
      PriorityQueue<Pair<Double, Pair<Couple<TrackNode>, TrackEdge>>> frontier = new PriorityQueue<>(
         (p1, p2) -> Double.compare((Double)p1.getFirst(), (Double)p2.getFirst())
      );
      ...
      if (connectionsFromInitial == null) {
         ...
         if (initialEdge == null) {
            ...
            while (!frontier.isEmpty()) {
               ...
               Pair<Couple<TrackNode>, TrackEdge> currentEntry = (Pair<Couple<TrackNode>, TrackEdge>)poll.getSecond();
               ...
               if (!(distance > (double)maxDistance)) {
                  ...
                  for (Entry<TrackNode, TrackEdge> entry : connectionsFrom.entrySet()) {
                     ...
                     if (!(currentDirection.dot(newDirection) < 0.875) && visited.add(entry.getValue())) {
                  ...
                  if (!validTargets.isEmpty()) {
                     for (Entry<TrackNode, TrackEdge> entryx : validTargets) {
                        TrackNode newNode = entryx.getKey();
                        TrackEdge newEdge = entryx.getValue();
