...
public class ScheduleRuntime {
   ...
   public void destinationReached() {
      if (this.state == ScheduleRuntime.State.IN_TRANSIT) {
         ...
         if (this.currentEntry < this.schedule.entries.size()) {
            List<List<ScheduleWaitCondition>> conditions = this.schedule.entries.get(this.currentEntry).conditions;
         ...
         if (!this.paused) {
            if (!this.train.derailed) {
               if (this.train.navigation.destination != null) {
                  this.ticksInTransit++;
               } else if (!this.checkEndOfScheduleReached()) {
                  if (this.cooldown-- <= 0) {
                     if (this.state != ScheduleRuntime.State.IN_TRANSIT) {
   ...
   public Collection<GlobalTrainDisplayData.TrainDeparturePrediction> submitPredictions() {
      ...
      if (this.state != ScheduleRuntime.State.POST_TRANSIT && current < entryCount) {
         ...
         if (destination != null) {
            ...
            if (this.predictionTicks.size() > current && this.train.navigation.distanceStartedAt != 0.0) {
               float predictedTime = (float)this.predictionTicks.get(current).intValue();
               if (predictedTime > 0.0F) {
                  predictedTime = (float)(
                     (double)predictedTime * Mth.clamp(this.train.navigation.distanceToDestination / this.train.navigation.distanceStartedAt, 0.0, 1.0)
                  );
      ...
      for (int i = 1; i < entryCount; i++) {
         ...
         if (this.schedule.entries.get(index).instruction instanceof ChangeTitleInstruction title) {
   ...
   private GlobalTrainDisplayData.TrainDeparturePrediction createPrediction(int index, String destination, String currentTitle, int time) {
      if (time == -2) {
         ...
         return new GlobalTrainDisplayData.TrainDeparturePrediction(this.train, time, Component.literal(text), destination);
      }
   }
   ...
   public CompoundTag write(Provider registries) {
      ...
      if (this.schedule != null) {
         tag.put("Schedule", this.schedule.write(registries));
      }

      NBTHelper.writeEnum(tag, "State", this.state);
      tag.putIntArray("ConditionProgress", this.conditionProgress);
      tag.put("ConditionContext", NBTHelper.writeCompoundList(this.conditionContext, CompoundTag::copy));
      tag.putIntArray("TransitTimes", this.predictionTicks);
      return tag;
   }

   public void read(Provider registries, CompoundTag tag) {
      this.reset();
      ...
      this.state = (ScheduleRuntime.State)NBTHelper.readEnum(tag, "State", ScheduleRuntime.State.class);
   ...
   public ItemStack returnSchedule(Provider registries) {
      if (this.schedule == null) {
         return ItemStack.EMPTY;
      } else {
         ItemStack stack = AllItems.SCHEDULE.asStack();
         this.schedule.savedProgress = this.currentEntry;
         stack.set(AllDataComponents.TRAIN_SCHEDULE, this.schedule.write(registries));
