...
public class FlapDisplayRenderer extends KineticBlockEntityRenderer<FlapDisplayBlockEntity> {
   ...
   protected void renderSafe(FlapDisplayBlockEntity be, float partialTicks, PoseStack ms, MultiBufferSource buffer, int light, int overlay) {
      ...
      if (be.isController) {
         ...
         ms.pushPose();
         ((PoseTransformStack)((PoseTransformStack)((PoseTransformStack)TransformStack.of(ms).center())
                  .rotateYDegrees(AngleHelper.horizontalAngle((Direction)be.getBlockState().getValue(FlapDisplayBlock.HORIZONTAL_FACING))))
               .uncenter())
            .translate(0.0F, 0.0F, -0.1875F);
         ...
         for (int j = 0; j < lines.size(); j++) {
            ...
            FlapDisplayRenderer.FlapDisplayRenderOutput renderOutput = new FlapDisplayRenderer.FlapDisplayRenderOutput(
               buffer, color, transform.pose(), light, j, !be.isSpeedRequirementFulfilled(), be.getLevel(), be.isLineGlowing(j)
            );
            ...
            if (buffer instanceof BufferSource bs) {
               ...
               bs.endBatch(texturedglyph.renderType(DisplayMode.NORMAL));
            }
   ...
   protected SuperByteBuffer getRotatedModel(FlapDisplayBlockEntity be, BlockState state) {
      return CachedBuffers.partialFacingVertical(AllPartialModels.SHAFTLESS_COGWHEEL, state, (Direction)state.getValue(FlapDisplayBlock.HORIZONTAL_FACING));
   }
   ...
   static class FlapDisplayRenderOutput implements FormattedCharSink {
      ...
      public float finish(int bgColor) {
         if (bgColor == 0) {
            ...
            VertexConsumer vertexconsumer = this.bufferSource.getBuffer(this.renderTypeOf(bakedglyph));
            bakedglyph.renderEffect(new Effect(-1.0F, 9.0F, this.section.size, -2.0F, 0.01F, r, g, b, a), this.pose, vertexconsumer, this.light);
            return this.x;
         }
      }
      ...
      private RenderType renderTypeOf(BakedGlyph bakedglyph) {
         return bakedglyph.renderType(DisplayMode.NORMAL);
      }
