...
public class FlapDisplayLayout {
   ...
   public CompoundTag write(Provider registries) {
      ...
      tag.put("Sections", NBTHelper.writeCompoundList(this.sections, s -> s.write(registries)));
      return tag;
   }
   ...
   public void read(CompoundTag tag, Provider registries) {
      ...
      if (!prevKey.equals(this.layoutKey)) {
         this.sections = NBTHelper.readCompoundList(sectionsTag, i -> FlapDisplaySection.load(i, registries));
      } else {
         MutableInt index = new MutableInt(0);
         NBTHelper.iterateCompoundList(sectionsTag, nbt -> this.sections.get(index.getAndIncrement()).update(nbt, registries));
      }
   }
