...
public class Navigation {
      ...
      if (this.destination != null) {
         ...
         if (this.train.graph != null) {
            if (this.waitingForSignal != null && this.currentSignalResolved()) {
               ...
               if (signal != null && signal.types.get((Boolean)this.waitingForSignal.getSecond()) == SignalBlock.SignalType.CROSS_SIGNAL) {
            ...
            TravellingPoint leadingPoint = !this.destinationBehindTrain
               ? this.train.carriages.get(0).getLeadingPoint()
               : this.train.carriages.get(this.train.carriages.size() - 1).getTrailingPoint();
            ...
            if (this.distanceToSignal > 0.0625) {
               ...
               this.signalScout
                  .travel(
                     this.train.graph,
                     (this.distanceToDestination + 50.0) * speedMod,
                     this.controlSignalScout(),
                     (distance, couple) -> {
                        ...
                        if (!crossSignalTracked && distance > scanDistance) {
                           ...
                           if (boundary == this.destination && ((GlobalStation)boundary).canApproachFrom((TrackNode)nodes.getSecond())) {
                              ...
                              if (signalEdgeGroup == null) {
                                 ...
                                 boolean occupied = !this.train.manualTick
                                    && (signal.isForcedRed((TrackNode)nodes.getSecond()) || signalEdgeGroup.isOccupiedUnless(this.train));
                     ...
                     (distance, edge) -> {
                        ...
                        if (turn == null
                           || !(vDistance > 0.0625)
                           || !(
                              ((Vec3)turn.axes.getFirst())
                                    .multiply(1.0, 0.0, 1.0)
                                    .distanceTo(((Vec3)turn.axes.getSecond()).multiply(1.0, 0.0, 1.0).scale(-1.0))
                                 < 0.015625
                           )
                           || !(vDistance / turn.getLength() < 0.225F)) {
         ...
         if (targetDistance < -10.0) {
            ...
            if (targetDistance < 10.0) {
               ...
               double speedRelativeToStation = this.train.speed * speedMod;
               if (speedRelativeToStation > maxApproachSpeed) {
                  this.train.speed = this.train.speed + (maxApproachSpeed - Math.abs(this.train.speed)) * 0.5 * speedMod;
                  return;
               }
            }
   ...
   private boolean currentSignalResolved() {
      if (this.train.manualTick) {
         ...
         if (signal == null) {
            return true;
         } else if (signal.types.get((Boolean)this.waitingForSignal.getSecond()) == SignalBlock.SignalType.CROSS_SIGNAL) {
            ...
            UUID groupId = (UUID)signal.groups.get((Boolean)this.waitingForSignal.getSecond());
            if (groupId == null) {
               return true;
            } else {
               SignalEdgeGroup signalEdgeGroupx = Create.RAILWAYS.signalEdgeGroups.get(groupId);
   ...
   public TravellingPoint.ITrackSelector control(TravellingPoint mp) {
      return this.destination == null
         ? mp.steer(this.train.manualSteer, new Vec3(0.0, 1.0, 0.0))
         : (graph, pair) -> this.navigateOptions(this.currentPath, graph, (List<Entry<TrackNode, TrackEdge>>)pair.getSecond());
   }
               ...
               this.search(Double.MAX_VALUE, maxCost, forward, destinations, (distance, cost, reachedVia, currentEntry, globalStation) -> {
                  for (GlobalStation destination : destinations) {
                     if (globalStation == destination) {
                        ...
                        for (TrackEdge edgeReached = edge; backTrack != null && edgeReached != initialEdge; backTrack = reachedVia.get(edgeReached)) {
                           if ((Boolean)backTrack.getFirst()) {
                              currentPath.add(0, toReach);
                           }

                           toReach = (Couple<TrackNode>)backTrack.getSecond();
            ...
            if (initialEdge != null) {
                     ...
                     if (!(point.getLocationOn(initialEdge) < initialEdge.getLength() - distanceToNode2)) {
                        ...
                        if (point instanceof GlobalStation) {
                           ...
                           if (station.canApproachFrom(initialNode2)
                              && stationTest.test(
                                 distanceToNode2,
                                 distanceToNode2 + (double)initialPenalty,
                                 reachedVia,
               ...
               if (!costRelevant || !(distanceToNode2 + (double)initialPenalty > maxCost)) {
                  ...
                  while (!frontier.isEmpty()) {
                     Navigation.FrontierEntry entry = frontier.poll();
                     if (visited.add(entry.edge)) {
                        ...
                        if (!(distance > maxDistance)) {
                           ...
                           if (entry.hasDestination) {
                              ...
                              if (signalData.hasPoints()) {
                                 for (TrackEdgePoint pointx : signalData.getPoints()) {
                                    if (pointx instanceof GlobalStation stationx
                                       && stationx.canApproachFrom(node2)
                                       && stationTest.test(distance, (double)penalty, reachedVia, Pair.of(Couple.create(node1, node2), edge), stationx)) {
                                       return;
                                    }
                                 }
                              }
                           ...
                           for (Entry<TrackNode, TrackEdge> connection : connectionsFrom.entrySet()) {
                              ...
                              if (newNode != node1 && edge.canTravelTo(connection.getValue())) {
                           ...
                           if (!validTargets.isEmpty()) {
                              ...
                              for (Entry<TrackNode, TrackEdge> target : validTargets) {
                                 if (validTypes.contains(target.getValue().getTrackMaterial().trackType)) {
                                    ...
                                    if (costRelevant) {
                                       newPenalty = penalty + penalties.getOrDefault(newEdge, 0);
                                    }
                                       ...
                                       for (TrackEdgePoint pointxx : signalData.getPoints()) {
                                          if (node2 != initialNode1 || !(pointxx.getLocationOn(newEdge) < edgeLength - distanceToNode2)) {
                                             ...
                                             if (pointxx instanceof SignalBoundary) {
                                                ...
                                                SignalEdgeGroup signalEdgeGroupx = Create.RAILWAYS.signalEdgeGroups.get(groupx);
                                             ...
                                             if (pointxx instanceof GlobalStation) {
                                                ...
                                                if (stationx.canApproachFrom(newNode)
                                                   && stationTest.test(
                                                      newDistance,
                                                      newDistance + (double)newPenalty,
                                                      reachedVia,
                                    ...
                                    if (!costRelevant || !(newDistance + (double)newPenalty > maxCost)) {
                                       ...
                                       if (destinations != null && !destinations.isEmpty()) {
                                          ...
                                          for (GlobalStation destination : destinations) {
                                             TrackNodeLocation destinationNode = (TrackNodeLocation)destination.edgeLocation.getFirst();
                                       ...
                                       frontier.add(
                                          new Navigation.FrontierEntry(newDistance, newPenalty, remainingDist, hasDestination, node2, newNode, newEdge)
                                       );
                                    }
                                 }
                              }
   ...
   public CompoundTag write(DimensionPalette dimensions) {
      ...
      if (this.destination == null) {
         ...
         if (this.waitingForSignal == null) {
            ...
            tag.putBoolean("BlockingSignalSide", (Boolean)this.waitingForSignal.getSecond());
   ...
   public void read(CompoundTag tag, TrackGraph graph, DimensionPalette dimensions) {
      ...
      if (this.destination != null) {
         ...
         NBTHelper.iterateCompoundList(
            tag.getList("Path", 10),
            c -> this.currentPath.add(Couple.deserializeEach(c.getList("Nodes", 10), c2 -> TrackNodeLocation.read(c2, dimensions)).map(graph::locateNode))
         );
         this.removeBrokenPathEntries();
