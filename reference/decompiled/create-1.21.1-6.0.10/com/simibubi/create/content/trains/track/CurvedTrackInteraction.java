...
public class CurvedTrackInteraction {
      ...
      if (player.getAbilities().mayBuild) {
         if (mc.options.keyAttack.isDown() && result != null) {
            ...
            if (blockState.isAir()) {
               ...
               breakTicks++;
               breakTimeout = 2;
               breakProgress = breakProgress + (creative ? 0.125F : blockState.getDestroyProgress(player, level, breakPos) / 8.0F);
               ...
               if (breakProgress >= 1.0F) {
                  CatnipServices.NETWORK
                     .sendToServer(new CurvedTrackDestroyPacket(breakPos, result.loc().curveTarget(), BlockPos.containing(result.vec()), false));
                  resetBreakProgress();
               }
            }
   ...
   public static boolean onClickInput(InteractionKeyMappingTriggered event) {
      TrackBlockOutline.BezierPointSelection result = TrackBlockOutline.result;
      if (result == null) {
         return false;
      } else {
         ...
         if (player != null && level != null) {
            if (event.isUseItem()) {
               ...
               if (AllItems.WRENCH.isIn(heldItem) && player.isShiftKeyDown()) {
                  CatnipServices.NETWORK
                     .sendToServer(
                        new CurvedTrackDestroyPacket(result.blockEntity().getBlockPos(), result.loc().curveTarget(), BlockPos.containing(result.vec()), true)
                     );
                  resetBreakProgress();
                  player.swing(InteractionHand.MAIN_HAND);
                  return true;
