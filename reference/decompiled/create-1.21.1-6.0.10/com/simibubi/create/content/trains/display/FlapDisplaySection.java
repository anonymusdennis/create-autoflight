...
public class FlapDisplaySection {
   ...
   public CompoundTag write(Provider registries) {
      ...
      if (this.component != null) {
         tag.putString("Text", Serializer.toJson(this.component, registries));
      }
   ...
   public static FlapDisplaySection load(CompoundTag tag, Provider registries) {
      ...
      if (!tag.contains("Text")) {
         ...
         section.component = Serializer.fromJson(tag.getString("Text"), registries);
         section.refresh(tag.getBoolean("Transition"));
         return section;
      }
   }

   public void update(CompoundTag tag, Provider registries) {
      String text = tag.getString("Text");
      if (!text.isEmpty()) {
         this.component = Serializer.fromJson(text, registries);
      }
   ...
   public static String[] getFlapCycle(String key) {
      return LOADED_FLAP_CYCLES.computeIfAbsent(key, k -> CreateLang.translateDirect("flap_display.cycles." + key).getString().split(";"));
   }
}
