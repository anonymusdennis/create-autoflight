         ...
         if (box != null && box.intersects(cameraEntity.getBoundingBox().inflate(50.0))) {
            ...
            for (Entry<TrackNodeLocation, TrackNode> nodeEntry : graph.nodes.entrySet()) {
               ...
               if (nodeLocation != null) {
                  ...
                  if (!(location.distanceTo(camera) > 50.0) && mc.level.dimension().equals(nodeLocation.dimension)) {
                     ...
                     if (map != null) {
                        ...
                        for (Entry<TrackNode, TrackEdge> entry : map.entrySet()) {
                           ...
                           if (edge.node1.location.dimension.equals(edge.node2.location.dimension)
                              && (other.hashCode() <= hashCode || !(other.location.getLocation().distanceTo(camera) <= 50.0))) {
                              ...
                              if (!edge.isTurn()) {
                                 if (signalData.hasSignalBoundaries()) {
                                       ...
                                       if (trackEdgePoint instanceof SignalBoundary) {
                                          ...
                                          if (group != null) {
                                             outliner.showLine(
                                                   ...
                                                   edge.getPosition(graph, prev + (prev == 0.0 ? 0.0 : 0.0625 / length)).add(yOffset),
                                                   edge.getPosition(graph, (prev = boundary.getLocationOn(edge) / length) - 0.0625 / length).add(yOffset)
                                                )
                                    ...
                                    if (prevBoundary != null) {
                                       ...
                                       if (group != null) {
                                          outliner.showLine(edge, edge.getPosition(graph, prev + 0.0625 / length).add(yOffset), endPoint.add(yOffset))
                                             .colored(group.color.get())
                                             .lineWidth(width);
                                       }
                                       continue;
                                    ...
                                    for (int i = 0; i <= turn.getSegmentCount(); i++) {
                                       ...
                                       if (previous != null) {
                                          if (currentBoundary != null && position > currentBoundaryPosition) {
                                             ...
                                             outliner.showLine(Pair.of(edge, previous), previous.add(yOffset), var59.add(yOffset))
            ...
            for (Entry<TrackNodeLocation, TrackNode> nodeEntry : graph.nodes.entrySet()) {
               ...
               if (nodeLocation != null) {
                  ...
                  if (!(location.distanceTo(camera) > 50.0) && mc.level.dimension().equals(nodeLocation.dimension)) {
                     ...
                     Outliner.getInstance().showLine(node.netId, v1, v2).colored(Color.mixColors(Color.WHITE, graph.color, 1.0F)).lineWidth(0.125F);
                     Map<TrackNode, TrackEdge> map = graph.connectionsByNode.get(node);
                     if (map != null) {
                        int hashCode = node.hashCode();
                        ...
                        for (Entry<TrackNode, TrackEdge> entry : map.entrySet()) {
                           ...
                           if (!edge.node1.location.dimension.equals(edge.node2.location.dimension)) {
                              ...
                              Outliner.getInstance().showLine(node.netId, v1, v2).colored(Color.mixColors(Color.WHITE, graph.color, 1.0F)).lineWidth(0.25F);
                              ...
                              if (!edge.isTurn()) {
                                 if (extended) {
                                    ...
                                    Outliner.getInstance()
                                       .showAABB(edge.edgeData, AABB.ofSize(materialPos, 0.25, 0.0, 0.25).move(0.0, -0.5, 0.0))
                                       .lineWidth(0.0625F)
                                       .colored(graph.color);
                                 }

                                 Outliner.getInstance()
                                    .showLine(edge, edge.getPosition(graph, 0.0).add(yOffset), edge.getPosition(graph, 1.0).add(yOffset))
                                    .colored(graph.color)
                                    .lineWidth(0.0625F);
                              } else {
                                 Vec3 previous = null;
