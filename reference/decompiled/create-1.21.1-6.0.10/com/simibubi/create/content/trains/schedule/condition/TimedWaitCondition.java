...
public abstract class TimedWaitCondition extends ScheduleWaitCondition {
   ...
   protected Component formatTime(boolean compact) {
      return compact
         ? Component.literal(this.getValue() + this.getUnit().suffix)
         : Component.literal(this.getValue() + " ").append(CreateLang.translateDirect(this.getUnit().key));
   }

   @Override
   public List<Component> getTitleAs(String type) {
      return ImmutableList.of(
         Component.translatable(this.getId().getNamespace() + ".schedule." + type + "." + this.getId().getPath()),
         CreateLang.translateDirect("schedule.condition.for_x_time", this.formatTime(false)).withStyle(ChatFormatting.DARK_AQUA)
      );
   }
   ...
   public void initConfigurationWidgets(ModularGuiLineBuilder builder) {
      ...
      builder.addSelectionScrollInput(
         36, 85, (i, l) -> i.forOptions(TimedWaitCondition.TimeUnit.translatedOptions()).titled(CreateLang.translateDirect("generic.timeUnit")), "TimeUnit"
      );
   }
   ...
   public MutableComponent getWaitingStatus(Level level, Train train, CompoundTag tag) {
      ...
      int num = (int)(
         showInMinutes ? Math.floor((double)((float)ticksUntilDeparture / 1200.0F)) : Math.ceil((double)((float)ticksUntilDeparture / 100.0F)) * 5.0
      );
      ...
      return CreateLang.translateDirect(
         "schedule.condition." + this.getId().getPath() + ".status", Component.literal(num + " ").append(CreateLang.translateDirect(key))
      );
   }
