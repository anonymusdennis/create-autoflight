...
public class TravellingPoint {
      ...
      return (graph, pair) -> {
         ...
         for (int i = 0; i < 20; i++) {
            for (int j = 0; j < validTargets.size(); j++) {
               Entry<TrackNode, TrackEdge> entryx = validTargets.get(j);
               List<Entry<TrackNode, TrackEdge>> frontier = frontiers.get(j);
               if (!frontier.isEmpty()) {
                  Entry<TrackNode, TrackEdge> currentEntry = frontier.remove(0);

                  for (Entry<TrackNode, TrackEdge> nextEntry : graph.getConnectionsFrom(currentEntry.getKey()).entrySet()) {
                     TrackEdge nextEdge = nextEntry.getValue();
                     if (visiteds.get(j).add(nextEdge) && currentEntry.getValue().canTravelTo(nextEdge)) {
            ...
            if (blockedLocation != null) {
               ...
               if (forward) {
                  while (this.position > edgeLength) {
                     ...
                     Entry<TrackNode, TrackEdge> entryx = validTargets.size() == 1
                        ? validTargets.get(0)
                        : trackSelector.apply(graph, Pair.of(true, validTargets));
                     if (entryx.getValue().getLength() == 0.0 && portalListener.test(Couple.create(this.node2.getLocation(), entryx.getKey().getLocation()))) {
                        traveled = distance - (this.position - edgeLength);
                        this.position = edgeLength;
                        this.blocked = true;
                        break;
                     ...
                     this.node2 = entryx.getKey();
                     this.edge = entryx.getValue();
                     this.position -= edgeLength;
                     collectedDistance += edgeLength;
                     if (this.edge.isTurn()) {
                        turnListener.accept(Double.valueOf(collectedDistance), this.edge);
                     }
                  ...
                  while (this.position < 0.0) {
                     ...
                     for (Entry<TrackNode, TrackEdge> entryxx : graph.getConnectionsFrom(this.node1).entrySet()) {
                        TrackNode newNode = entryxx.getKey();
                        if (newNode != this.node2 && graph.getConnectionsFrom(newNode).get(this.node1).canTravelTo(this.edge)) {
                     ...
                     Entry<TrackNode, TrackEdge> entryxxx = validTargets.size() == 1
                        ? validTargets.get(0)
                        : trackSelector.apply(graph, Pair.of(false, validTargets));
                     if (entryxxx.getValue().getLength() == 0.0
                        && portalListener.test(Couple.create(entryxxx.getKey().getLocation(), this.node1.getLocation()))) {
                        traveled = distance - this.position;
                        this.position = 0.0;
                        this.blocked = true;
                        break;
   ...
   protected Double edgeTraversedFrom(
      ...
      if (this.edge.isTurn()) {
         turnListener.accept(Double.valueOf(Math.max(0.0, totalDistance)), this.edge);
      }
      ...
      for (int i = 0; i < edgePoints.size(); i++) {
         ...
         if (forward ? !(locationOn < from) && !(locationOn >= to) : !(locationOn <= from) && !(locationOn > to)) {
            ...
            if (edgePointListener.test(Double.valueOf(totalDistance + distance), Pair.of(nextBoundary, forward ? nodes : nodes.swap()))) {
               return locationOn;
            }
         }
      }
   ...
   public void migrateTo(List<TrackGraphLocation> locations) {
      ...
      this.node1 = graph.locateNode((TrackNodeLocation)location.edge.getFirst());
      this.node2 = graph.locateNode((TrackNodeLocation)location.edge.getSecond());
   ...
   public static TravellingPoint read(CompoundTag tag, TrackGraph graph, DimensionPalette dimensions) {
      if (graph == null) {
         ...
         Couple<TrackNode> locs = tag.contains("Nodes")
            ? Couple.deserializeEach(tag.getList("Nodes", 10), c -> TrackNodeLocation.read(c, dimensions)).map(graph::locateNode)
         ...
         if (locs.either(Objects::isNull)) {
            ...
            return new TravellingPoint(
               (TrackNode)locs.getFirst(),
               (TrackNode)locs.getSecond(),
               graph.getConnectionsFrom((TrackNode)locs.getFirst()).get(locs.getSecond()),
               position,
