...
public class TimeOfDayCondition extends ScheduleWaitCondition {
   ...
   public boolean tickCompletion(Level level, Train train, CompoundTag context) {
      ...
      int targetTicks = (int)(
         ((double)((targetHour + 18) % 24 * 1000) + Math.ceil((double)((float)targetMinute / 60.0F * 1000.0F))) % (double)this.getRotation()
      );
   ...
   public List<Component> getTitleAs(String type) {
      return ImmutableList.of(
         ...
         this.getDigitalDisplay(this.intData("Hour"), this.intData("Minute"), false)
   ...
   public boolean renderSpecialIcon(GuiGraphics graphics, int x, int y) {
      ...
      ResourceLocation location = ResourceLocation.withDefaultNamespace(
         "textures/item/clock_" + this.twoDigits(Mth.clamp((int)(progress * 64.0F), 0, 63)) + ".png"
      );
   ...
   public void initConfigurationWidgets(ModularGuiLineBuilder builder) {
      ...
      ((ScrollInput)hourInput.getValue()).titled(CreateLang.translateDirect("generic.daytime.hour")).calling(t -> {
      ...
      ((ScrollInput)minuteInput.getValue()).titled(CreateLang.translateDirect("generic.daytime.minute")).calling(t -> {
   ...
   public MutableComponent getWaitingStatus(Level level, Train train, CompoundTag tag) {
      ...
      return CreateLang.translateDirect("schedule.condition.time_of_day.status").append(this.getDigitalDisplay(departingHour, departingMinute, false));
   }
}
