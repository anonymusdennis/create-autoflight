...
public class TrainHatInfoReloadListener {
   ...
   private static void registerOffsetOverrides(ResourceManager manager) {
      ...
      FileToIdConverter converter = FileToIdConverter.json("train_hat_info");
      converter.listMatchingResources(manager)
         .forEach(
            (location, resource) -> {
               ...
               if (!BuiltInRegistries.ENTITY_TYPE.containsKey(entityName)) {
                  ...
                  try (BufferedReader reader = resource.openAsReader()) {
                     ...
                     ENTITY_INFO_MAP.put(
                        (EntityType<?>)BuiltInRegistries.ENTITY_TYPE.get(entityName),
                        (TrainHatInfo)TrainHatInfo.CODEC.parse(JsonOps.INSTANCE, json).resultOrPartial(Create.LOGGER::error).orElseThrow()
                     );
                  } catch (Exception var9) {
                     Create.LOGGER.error("Failed to read train hat info for entity {}!", entityName, var9);
                  }
               }
            }
         );
