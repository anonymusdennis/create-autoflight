...
public class TrackNodeLocation extends Vec3i {
   ...
   public TrackNodeLocation(double x, double y, double z) {
      super(Mth.floor((double)Math.round(x * 2.0)), Mth.floor(y) * 2, Mth.floor((double)Math.round(z * 2.0)));
   }
   ...
   public static TrackNodeLocation read(CompoundTag tag, DimensionPalette dimensions) {
      TrackNodeLocation location = fromPackedPos(
         tag.contains("Pos") ? NBTHelper.readBlockPos(tag, "Pos") : new BlockPos(tag.getInt("X"), tag.getInt("Y"), tag.getInt("Z"))
      );
      if (dimensions != null) {
