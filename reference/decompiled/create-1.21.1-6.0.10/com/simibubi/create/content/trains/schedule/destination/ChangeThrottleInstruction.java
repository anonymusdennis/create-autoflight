...
public class ChangeThrottleInstruction extends ScheduleInstruction {
   ...
   private MutableComponent formatted() {
      return Component.literal(this.intData("Value") + "%");
   }
   ...
   public List<Component> getTitleAs(String type) {
      return ImmutableList.of(
         CreateLang.translateDirect("schedule." + type + "." + this.getId().getPath() + ".summary", this.formatted().withStyle(ChatFormatting.WHITE))
   ...
   public void initConfigurationWidgets(ModularGuiLineBuilder builder) {
      builder.addScrollInput(0, 50, (si, l) -> {
         si.withRange(5, 101).withStepFunction(c -> c.shift ? 25 : 5).titled(CreateLang.translateDirect("schedule.instruction.throttle_edit_box"));
         l.withSuffix("%");
      }, "Value");
   }
   ...
   public DiscoveredPath start(ScheduleRuntime runtime, Level level) {
      runtime.train.throttle = (double)this.getThrottle();
