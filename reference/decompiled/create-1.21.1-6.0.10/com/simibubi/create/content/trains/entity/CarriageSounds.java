...
public class CarriageSounds {
   ...
   public CarriageSounds(CarriageContraptionEntity entity) {
      this.entity = entity;
      this.bogeySounds = entity.getCarriage()
         .bogeys
         .map(bogey -> bogey != null && bogey.getStyle() != null ? bogey.getStyle().soundEvent.get() : AllSoundEvents.TRAIN2.getMainEvent());
         ...
         if (leadingAnchor != null && trailingAnchor != null) {
            ...
            if (this.entity.carriageIndex == 0) {
               train.accumulatedSteamRelease = (float)Math.min(
                  (double)train.accumulatedSteamRelease + Math.min(0.5, Math.abs(contraptionMotion.length() / 10.0)), 10.0
               );
            }
            ...
            CarriageBogey relevantBogey = (CarriageBogey)bogeys.get(distance1 > distance2);
            ...
            this.speedFactor.chase(contraptionMotion.length(), 0.25, Chaser.exp(0.05F));
            this.distanceFactor.chase(Mth.clampedLerp(100.0, 0.0, (distance - 3.0) / 64.0), 0.25, Chaser.exp(50.0));
            this.approachFactor.chase(Mth.clampedLerp(50.0, 200.0, 0.5 * (dot + 1.0)), 0.25, Chaser.exp(10.0));
            this.seatCrossfade.chase(camEntity.getVehicle() instanceof CarriageContraptionEntity ? 1.0 : 0.0, 0.1F, Chaser.EXP);
            ...
            float volume = Math.min(
               Math.min(this.speedFactor.getValue(), this.distanceFactor.getValue() / 100.0F), this.approachFactor.getValue() / 300.0F + 0.0125F
            );
            if (this.entity.carriageIndex == 0) {
            ...
            if (!this.arrived && this.speedFactor.getValue() < 0.002F && train.accumulatedSteamRelease > 1.0F) {
               ...
               this.entity
                  .level()
                  .playLocalSound(
                     soundLocation.x, soundLocation.y, soundLocation.z, SoundEvents.LAVA_EXTINGUISH, SoundSource.NEUTRAL, 0.25F * releaseVolume, 0.78F, false
                  );
               this.entity
                  .level()
                  .playLocalSound(
   ...
   private CarriageSounds.LoopingSound playIfMissing(Minecraft mc, CarriageSounds.LoopingSound loopingSound, SoundEvent sound, boolean continuouslyShowSubtitle) {
      if (loopingSound == null) {
         loopingSound = new CarriageSounds.LoopingSound(sound, SoundSource.NEUTRAL, continuouslyShowSubtitle);
      ...
      public void tick() {
         if (this.repeatSubtitle) {
            ...
            if (this.subtitleTimer == 20) {
               OVERLAY.onPlaySound(this, this.weighedSoundEvents, (float)this.sound.getAttenuationDistance());
