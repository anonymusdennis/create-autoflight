...
public class EntityLauncher {
   ...
   public void applyMotion(Entity entity, Direction facing) {
      ...
      motionVec = VecHelper.rotate(motionVec, (double)AngleHelper.horizontalAngle(facing), Axis.Y);
   ...
   public Vec3 getGlobalPos(double t, Direction d, BlockPos launcher) {
      ...
      Vec3 vec = new Vec3(
         0.0,
         (double)yt + (double)((float)this.verticalDistance - yt) * correctionStrength * 0.5,
         (double)xt + (double)((float)this.horizontalDistance - xt) * correctionStrength
      );
      return VecHelper.rotate(vec, (double)(180.0F + AngleHelper.horizontalAngle(d)), Axis.Y).add(start);
   }

   public Vec3 getGlobalVelocity(double t, Direction d, BlockPos launcher) {
      return VecHelper.rotate(new Vec3(0.0, (double)this.dy(t), (double)this.dx(t)), (double)(180.0F + AngleHelper.horizontalAngle(d)), Axis.Y);
   }
   ...
   protected void recalculateTrajectory() {
      ...
      double xError = -7.0E-4 * Math.pow(xTarget + 0.5, 2.0)
         + 0.484
         - Math.min(5.0, yTarget) / 5.0 * Math.min(1.0, 0.076 * xTarget - 0.0014 * xTarget * xTarget);
