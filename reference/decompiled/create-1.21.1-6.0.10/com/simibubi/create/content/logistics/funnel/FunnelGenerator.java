...
public class FunnelGenerator extends SpecialBlockStateGen {
   ...
   protected int getYRotation(BlockState state) {
      return this.horizontalAngle((Direction)state.getValue(FunnelBlock.FACING)) + 180;
   }
   ...
   public <T extends Block> ModelFile getModel(DataGenContext<Block, T> c, RegistrateBlockstateProvider p, BlockState s) {
      ...
      Direction facing = (Direction)s.getValue(FunnelBlock.FACING);
      ...
      BlockModelBuilder model = (BlockModelBuilder)((BlockModelBuilder)((BlockModelBuilder)((BlockModelBuilder)((BlockModelBuilder)p.models()
                     .withExistingParent("block/" + this.type + "_funnel_" + parent + extracting + powered, p.modLoc(prefix + "block_" + parent)))
                  .texture("particle", this.blockTexture))
               .texture("base", p.modLoc(prefix + this.type + "_funnel")))
            .texture("redstone", p.modLoc(prefix + this.type + "_funnel" + powered)))
         .texture("direction", p.modLoc(prefix + this.type + "_funnel" + extracting));
      return horizontal
         ? model.texture("block", this.blockTexture)
         : ((BlockModelBuilder)model.texture("frame", p.modLoc(prefix + this.type + "_funnel_frame"))).texture("open", p.modLoc(prefix + "funnel" + closed));
   }
   ...
   public static NonNullBiConsumer<DataGenContext<Item, FunnelItem>, RegistrateItemModelProvider> itemModel(String type) {
      ...
      return (c, p) -> ((ItemModelBuilder)((ItemModelBuilder)((ItemModelBuilder)((ItemModelBuilder)((ItemModelBuilder)p.withExistingParent(
                           "item/" + type + "_funnel", p.modLoc("block/funnel/item")
                        ))
