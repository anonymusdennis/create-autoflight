...
public class LogisticallyLinkedBlockItem extends BlockItem {
   ...
   public static UUID networkFromStack(ItemStack pStack) {
      CompoundTag tag = ((CustomData)pStack.getOrDefault(DataComponents.BLOCK_ENTITY_DATA, CustomData.EMPTY)).copyTag();
      return !tag.hasUUID("Freq") ? null : tag.getUUID("Freq");
   }

   public void appendHoverText(
      @NotNull ItemStack stack, @NotNull TooltipContext tooltipContext, @NotNull List<Component> tooltipComponents, @NotNull TooltipFlag tooltipFlag
   ) {
      ...
      if (tag.hasUUID("Freq")) {
         ...
         CreateLang.translate("logistically_linked.tooltip_clear").style(ChatFormatting.GRAY).addTo(tooltipComponents);
      }
   }
   ...
   public InteractionResult useOn(UseOnContext pContext) {
      ...
      if (player == null) {
         ...
         if (link != null) {
            ...
            } else if (!link.mayInteractMessage(player)) {
               return InteractionResult.SUCCESS;
            } else {
            ...
            if (!level.isClientSide && useOn != InteractionResult.FAIL) {
               player.displayClientMessage(
                  tuned ? CreateLang.translateDirect("logistically_linked.connected") : CreateLang.translateDirect("logistically_linked.new_network_started"),
                  true
               );
               return useOn;
            } else {
