...
public class TableClothModel extends BakedModelWrapperWithData {
   private static final ModelProperty<TableClothModel.CullData> CULL_PROPERTY = new ModelProperty();
   ...
   private List<BakedQuad> getCorner(TableClothBlock block, int corner, @NotNull RandomSource rand, @Nullable RenderType renderType) {
      if (!CORNERS.containsKey(block)) {
         ...
         for (PartialModel pm : List.of(
            AllPartialModels.TABLE_CLOTH_SW, AllPartialModels.TABLE_CLOTH_NW, AllPartialModels.TABLE_CLOTH_NE, AllPartialModels.TABLE_CLOTH_SE
         )) {
   ...
   protected Builder gatherModelData(Builder builder, BlockAndTintGetter world, BlockPos pos, BlockState state, ModelData blockEntityData) {
      ...
      return culledSides.isEmpty() ? builder : builder.with(CULL_PROPERTY, new TableClothModel.CullData(EnumSet.copyOf(culledSides)));
   }
   ...
   public List<BakedQuad> getQuads(
      ...
      if (side != null && side.getAxis() != Axis.Y) {
         TableClothModel.CullData cullData = (TableClothModel.CullData)extraData.get(CULL_PROPERTY);
         if (cullData != null && cullData.culled().contains(side.getClockWise())) {
            return mainQuads;
         } else if (state != null && state.getBlock() instanceof TableClothBlock dcb) {
