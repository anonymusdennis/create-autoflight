...
public class FunnelMovementBehaviour implements MovementBehaviour {
   ...
   public Vec3 getActiveAreaOffset(MovementContext context) {
      ...
      return facing != Direction.UP ? vec.scale(context.state.getValue(FunnelBlock.EXTRACTING) ? 0.15 : 0.65) : vec.scale(0.65);
   }

   @Override
   public void visitNewPosition(MovementContext context, BlockPos pos) {
      MovementBehaviour.super.visitNewPosition(context, pos);
      if ((Boolean)context.state.getValue(FunnelBlock.EXTRACTING)) {
      ...
      for (Entity entity : items) {
         if (entity.isAlive()) {
            ...
            if (filter.test(context.world, toInsert)) {
               ItemStack remainder = ItemHandlerHelper.insertItemStacked(context.contraption.getStorage().getAllItems(), toInsert, false);
