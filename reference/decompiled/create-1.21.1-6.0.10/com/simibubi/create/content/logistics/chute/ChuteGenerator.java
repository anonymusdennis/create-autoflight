...
public class ChuteGenerator extends SpecialBlockStateGen {
   ...
   protected int getYRotation(BlockState state) {
      return this.horizontalAngle((Direction)state.getValue(ChuteBlock.FACING));
   }

   @Override
   public <T extends Block> ModelFile getModel(DataGenContext<Block, T> ctx, RegistrateBlockstateProvider prov, BlockState state) {
      boolean horizontal = state.getValue(ChuteBlock.FACING) != Direction.DOWN;
      ChuteBlock.Shape shape = (ChuteBlock.Shape)state.getValue(ChuteBlock.SHAPE);
      if (horizontal) {
         return shape == ChuteBlock.Shape.INTERSECTION
            ? AssetLookup.partialBaseModel(ctx, prov, "diagonal", "intersection")
            : (
               shape == ChuteBlock.Shape.ENCASED
                  ? AssetLookup.partialBaseModel(ctx, prov, "diagonal", "encased")
                  : AssetLookup.partialBaseModel(ctx, prov, "diagonal")
            );
      } else {
         return shape == ChuteBlock.Shape.NORMAL
