...
public record AutoRequestData(PackageOrderWithCrafts encodedRequest, String encodedTargetAddress, BlockPos targetOffset, String targetDim, boolean isValid) {
   public static final Codec<AutoRequestData> CODEC = RecordCodecBuilder.create(
   ...
   public void writeToItem(BlockPos position, ItemStack itemStack) {
      AutoRequestData.Mutable mutable = new AutoRequestData.Mutable(this);
      mutable.targetOffset = position.offset(this.targetOffset);
   ...
   public static AutoRequestData readFromItem(Level level, Player player, BlockPos position, ItemStack itemStack) {
      ...
      if (requestData == null) {
         ...
         AutoRequestData.Mutable mutable = new AutoRequestData.Mutable(requestData);
         mutable.targetOffset = mutable.targetOffset.subtract(position);
         mutable.isValid = mutable.targetOffset.closerThan(BlockPos.ZERO, 128.0) && requestData.targetDim.equals(level.dimension().location().toString());
         if (player != null) {
            CreateLang.translate(mutable.isValid ? "redstone_requester.keeper_connected" : "redstone_requester.keeper_too_far_away")
