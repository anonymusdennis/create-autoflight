...
public class FluidValveRenderer extends KineticBlockEntityRenderer<FluidValveBlockEntity> {
   ...
   protected void renderSafe(FluidValveBlockEntity be, float partialTicks, PoseStack ms, MultiBufferSource buffer, int light, int overlay) {
      if (!VisualizationManager.supportsVisualization(be.getLevel())) {
         ...
         Direction facing = (Direction)blockState.getValue(FluidValveBlock.FACING);
         ...
         ((SuperByteBuffer)((SuperByteBuffer)((SuperByteBuffer)((SuperByteBuffer)((SuperByteBuffer)pointer.center())
                        .rotateYDegrees(AngleHelper.horizontalAngle(facing)))
                     .rotateXDegrees(facing == Direction.UP ? 0.0F : (facing == Direction.DOWN ? 180.0F : 90.0F)))
                  .rotateYDegrees((float)pointerRotationOffset + pointerRotation))
               .uncenter())
            .light(light)
