...
public class PipeAttachmentModel extends BakedModelWrapperWithData {
   ...
   public ChunkRenderTypeSet getRenderTypes(@NotNull BlockState state, @NotNull RandomSource rand, @NotNull ModelData data) {
      ...
      if (data.has(PIPE_PROPERTY)) {
         PipeAttachmentModel.PipeModelData pipeData = (PipeAttachmentModel.PipeModelData)data.get(PIPE_PROPERTY);
         ...
         for (Direction d : Iterate.directions) {
            ...
            for (FluidTransportBehaviour.AttachmentTypes.ComponentPartials partial : type.partials) {
               ChunkRenderTypeSet attachmentRenderTypeSet = AllPartialModels.PIPE_ATTACHMENTS.get(partial).get(d).get().getRenderTypes(state, rand, data);
   ...
   public TriState useAmbientOcclusion(BlockState state, ModelData data, RenderType renderType) {
      return this.ao ? TriState.TRUE : TriState.FALSE;
   }
   ...
   private void addQuads(
      ...
      RandomSource rand,
      ModelData data,
      PipeAttachmentModel.PipeModelData pipeData,
      ...
      for (Direction d : Iterate.directions) {
         ...
         for (FluidTransportBehaviour.AttachmentTypes.ComponentPartials partial : type.partials) {
            quads.addAll(AllPartialModels.PIPE_ATTACHMENTS.get(partial).get(d).get().getQuads(state, side, rand, data, renderType));
         }
      }
