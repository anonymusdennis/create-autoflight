...
public class FillingBySpout {
   public static boolean canItemBeFilled(Level world, ItemStack stack) {
      ...
      if (assemblyRecipe.isPresent()) {
         ...
         return AllRecipeTypes.FILLING.find(input, world).isPresent() ? true : GenericItemFilling.canItemBeFilled(world, stack);
      }
   }
   ...
   public static int getRequiredAmountForItem(Level world, ItemStack stack, FluidStack availableFluid) {
      SingleRecipeInput input = new SingleRecipeInput(stack);
      Optional<RecipeHolder<FillingRecipe>> assemblyRecipe = SequencedAssemblyRecipe.getRecipe(
         world, input, AllRecipeTypes.FILLING.getType(), FillingRecipe.class, matchItemAndFluid(world, availableFluid, input)
      );
      if (assemblyRecipe.isPresent()) {
         SizedFluidIngredient requiredFluid = ((FillingRecipe)assemblyRecipe.get().value()).getRequiredFluid();
         if (requiredFluid.ingredient().test(availableFluid)) {
            return requiredFluid.amount();
         }
      }

      for (RecipeHolder<Recipe<SingleRecipeInput>> recipe : world.getRecipeManager().getRecipesFor(AllRecipeTypes.FILLING.getType(), input, world)) {
   ...
   public static ItemStack fillItem(Level level, int requiredAmount, ItemStack stack, FluidStack availableFluid) {
      ...
      RecipeHolder<FillingRecipe> fillingRecipe = SequencedAssemblyRecipe.getRecipe(
            level, input, AllRecipeTypes.FILLING.getType(), FillingRecipe.class, matchItemAndFluid(level, availableFluid, input)
         )
         .filter(fr -> ((FillingRecipe)fr.value()).getRequiredFluid().test(toFill))
         .orElseGet(() -> {
            for (RecipeHolder<Recipe<SingleRecipeInput>> recipe : level.getRecipeManager().getRecipesFor(AllRecipeTypes.FILLING.getType(), input, level)) {
