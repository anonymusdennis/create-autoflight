...
public class PotionMixingRecipes {
   ...
   private static List<RecipeHolder<MixingRecipe>> createRecipesImpl(Level level) {
      ...
      for (Item containerx : allowedSupportedContainers) {
         ...
         for (Mix<Potion> mix : ((PotionBrewingAccessor)potionBrewing).create$getPotionMixes()) {
      ...
      for (Mix<Item> mix : ((PotionBrewingAccessor)potionBrewing).create$getContainerMixes()) {
         Item from = (Item)mix.from().value();
         if (allowedSupportedContainers.contains(from)) {
            Item to = (Item)mix.to().value();
