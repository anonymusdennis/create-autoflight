...
public class PressingBehaviour extends BeltProcessingBehaviour {
   ...
   public void read(CompoundTag compound, Provider registries, boolean clientPacket) {
      ...
      if (clientPacket) {
         NBTHelper.iterateCompoundList(compound.getList("ParticleItems", 10), c -> this.particleItems.add(ItemStack.parseOptional(registries, c)));
         this.spawnParticles();
      }
   }
   ...
   public void write(CompoundTag compound, Provider registries, boolean clientPacket) {
      ...
      if (clientPacket) {
         compound.put("ParticleItems", NBTHelper.writeCompoundList(this.particleItems, s -> (CompoundTag)s.saveOptional(registries)));
   ...
   public float getRenderedHeadOffset(float partialTicks) {
      if (!this.running) {
         ...
         return runningTicks < 160
            ? (float)Mth.clamp(Math.pow((double)(ticks / 240.0F * 2.0F), 3.0), 0.0, 1.0)
            : Mth.clamp((240.0F - ticks) / 240.0F * 3.0F, 0.0F, 1.0F);
      }
   }
         ...
         if (level.isClientSide && this.runningTicks == -120) {
            ...
            if (this.runningTicks == 120 && this.specifics.getKineticSpeed() != 0.0F) {
               ...
               if (level.getBlockState(worldPosition.below(2)).getSoundType() == SoundType.WOOL) {
                  ...
                  AllSoundEvents.MECHANICAL_PRESS_ACTIVATION
                     .playOnServer(level, worldPosition, 0.5F, 0.75F + Math.abs(this.specifics.getKineticSpeed()) / 1024.0F);
               }
