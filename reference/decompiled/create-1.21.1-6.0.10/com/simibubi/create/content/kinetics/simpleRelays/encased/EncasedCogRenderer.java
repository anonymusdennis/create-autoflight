...
public class EncasedCogRenderer extends KineticBlockEntityRenderer<SimpleKineticBlockEntity> {
   ...
   public static EncasedCogRenderer small(Context context) {
      return new EncasedCogRenderer(context, false);
   }

   public static EncasedCogRenderer large(Context context) {
      return new EncasedCogRenderer(context, true);
   }

   public EncasedCogRenderer(Context context, boolean large) {
      super(context);
      this.large = large;
   }
   ...
   protected void renderSafe(SimpleKineticBlockEntity be, float partialTicks, PoseStack ms, MultiBufferSource buffer, int light, int overlay) {
      ...
      if (!VisualizationManager.supportsVisualization(be.getLevel())) {
         ...
         if (blockState.getBlock() instanceof IRotate def) {
            ...
            float angle = this.large ? BracketedKineticBlockEntityRenderer.getAngleForLargeCogShaft(be, axis) : getAngleForBe(be, pos, axis);
   ...
   protected SuperByteBuffer getRotatedModel(SimpleKineticBlockEntity be, BlockState state) {
      return CachedBuffers.partialFacingVertical(
         ...
         state,
         Direction.fromAxisAndDirection((Axis)state.getValue(EncasedCogwheelBlock.AXIS), AxisDirection.POSITIVE)
      );
   }
}
