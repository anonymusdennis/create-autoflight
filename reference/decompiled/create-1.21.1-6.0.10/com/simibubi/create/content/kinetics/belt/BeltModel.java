...
public class BeltModel extends BakedModelWrapper<BakedModel> {
   ...
   public TextureAtlasSprite getParticleIcon(ModelData data) {
      if (!data.has(CASING_PROPERTY)) {
         ...
         BeltBlockEntity.CasingType type = (BeltBlockEntity.CasingType)data.get(CASING_PROPERTY);
         return type != BeltBlockEntity.CasingType.NONE && type != BeltBlockEntity.CasingType.BRASS
            ? AllSpriteShifts.ANDESITE_CASING.getOriginal()
   ...
   public List<BakedQuad> getQuads(BlockState state, Direction side, RandomSource rand, ModelData extraData, RenderType renderType) {
      ...
      if (!extraData.has(CASING_PROPERTY)) {
         ...
         BeltBlockEntity.CasingType type = (BeltBlockEntity.CasingType)extraData.get(CASING_PROPERTY);
         ...
         if (type != BeltBlockEntity.CasingType.NONE && (!brassCasing || cover)) {
            ...
            if (cover) {
               boolean alongX = ((Direction)state.getValue(BeltBlock.HORIZONTAL_FACING)).getAxis() == Axis.X;
               BakedModel coverModel = (brassCasing
                     ? (alongX ? AllPartialModels.BRASS_BELT_COVER_X : AllPartialModels.BRASS_BELT_COVER_Z)
                     : (alongX ? AllPartialModels.ANDESITE_BELT_COVER_X : AllPartialModels.ANDESITE_BELT_COVER_Z))
                  .get();
