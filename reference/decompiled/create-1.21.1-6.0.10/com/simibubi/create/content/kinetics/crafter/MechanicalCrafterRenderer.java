...
public class MechanicalCrafterRenderer extends SafeBlockEntityRenderer<MechanicalCrafterBlockEntity> {
      ...
      if (be.phase == MechanicalCrafterBlockEntity.Phase.IDLE) {
         ...
         if (be.phase == MechanicalCrafterBlockEntity.Phase.CRAFTING) {
            ...
            Vec3 centering = new Vec3(
                  (double)((float)(-items.minX) + (float)(-items.width + 1) / 2.0F), (double)((float)(-items.minY) + (float)(-items.height + 1) / 2.0F), 0.0
               )
               .scale((double)earlyProgress);
            ms.translate(centering.x * 0.5, centering.y * 0.5, 0.0);
            distance += (-4.0F * (progress - 0.5F) * (progress - 0.5F) + 1.0F) * 0.25F;
         }

         boolean onlyRenderFirst = be.phase == MechanicalCrafterBlockEntity.Phase.INSERTING
            || be.phase == MechanicalCrafterBlockEntity.Phase.CRAFTING && be.countDown < 1000;
   ...
   public void renderFast(MechanicalCrafterBlockEntity be, float partialTicks, PoseStack ms, MultiBufferSource buffer, int light) {
      ...
      if (!VisualizationManager.supportsVisualization(be.getLevel())) {
         ...
         superBuffer.rotateCentered(
            (float)(
               ((Direction)blockState.getValue(HorizontalKineticBlock.HORIZONTAL_FACING)).getAxis() != net.minecraft.core.Direction.Axis.X ? 0.0 : Math.PI / 2
            ),
            Direction.UP
         );
      ...
      if ((be.covered || be.phase != MechanicalCrafterBlockEntity.Phase.IDLE)
         && be.phase != MechanicalCrafterBlockEntity.Phase.CRAFTING
         && be.phase != MechanicalCrafterBlockEntity.Phase.INSERTING) {
   ...
   private SuperByteBuffer renderAndTransform(PartialModel renderBlock, BlockState crafterState) {
      ...
      float xRot = (float)((Pointing)crafterState.getValue(MechanicalCrafterBlock.POINTING)).getXRotation();
