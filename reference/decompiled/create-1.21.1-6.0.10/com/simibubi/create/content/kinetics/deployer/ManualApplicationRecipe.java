...
public class ManualApplicationRecipe extends ItemApplicationRecipe {
   @SubscribeEvent
   public static void manualApplicationRecipesApplyInWorld(RightClickBlock event) {
      ...
      if (!heldItem.isEmpty()) {
         if (!blockState.isAir()) {
            if (!event.isCanceled()) {
               ...
               Optional<RecipeHolder<Recipe<RecipeWrapper>>> foundRecipe = level.getRecipeManager().getAllRecipesFor(type).stream().filter(r -> {
   ...
   public static RecipeHolder<DeployerApplicationRecipe> asDeploying(RecipeHolder<?> recipe) {
      ...
      ItemApplicationRecipe.Builder<DeployerApplicationRecipe> builder = new ItemApplicationRecipe.Builder<>(DeployerApplicationRecipe::new, id)
         .require((Ingredient)mar.ingredients.get(0))
         .require((Ingredient)mar.ingredients.get(1));
   ...
   public boolean testBlock(BlockState in) {
      return ((Ingredient)this.ingredients.get(0)).test(new ItemStack(in.getBlock().asItem()));
   }
