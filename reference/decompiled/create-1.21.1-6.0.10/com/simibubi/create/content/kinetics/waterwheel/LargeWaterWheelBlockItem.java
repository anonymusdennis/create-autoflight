...
public class LargeWaterWheelBlockItem extends BlockItem {
   ...
   public void showBounds(BlockPlaceContext context) {
      ...
      Axis axis = ((LargeWaterWheelBlock)this.getBlock()).getAxisForPlacement(context);
      ...
      if (context.getPlayer() instanceof LocalPlayer localPlayer) {
         Outliner.getInstance().showAABB(Pair.of("waterwheel", pos), new AABB(pos).inflate(1.0).deflate(contract.x, contract.y, contract.z)).colored(-41620);
         CreateLang.translate("large_water_wheel.not_enough_space").color(-41620).sendStatus(localPlayer);
      }
   }
}
