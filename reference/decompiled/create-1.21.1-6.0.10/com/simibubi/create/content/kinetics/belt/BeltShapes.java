...
public class BeltShapes {
   ...
   private static VoxelShape getBeltShape(BlockState state) {
      ...
      if (slope == BeltSlope.VERTICAL) {
         return part != BeltPart.MIDDLE && part != BeltPart.PULLEY
            ? (part == BeltPart.START ? VERTICAL_START : VERTICAL_END).get(facing)
            : VERTICAL_FULL.get(axis);
   ...
   private static VoxelShape getCasingShape(BlockState state) {
      if (!(Boolean)state.getValue(BeltBlock.CASING)) {
         return Shapes.empty();
      } else {
         Direction facing = (Direction)state.getValue(BeltBlock.HORIZONTAL_FACING);
         BeltPart part = (BeltPart)state.getValue(BeltBlock.PART);
   ...
   private static class VerticalBeltShaper extends VoxelShaper {
      public static VoxelShaper make(VoxelShape southBeltShape) {
         return forDirectionsWithRotation(
            rotatedCopy(southBeltShape, new Vec3(-90.0, 0.0, 0.0)),
            Direction.SOUTH,
            Plane.HORIZONTAL,
            direction -> new Vec3(direction.getAxisDirection() == AxisDirection.POSITIVE ? 0.0 : 180.0, (double)(-direction.toYRot()), 0.0)
         );
      }
   }
}
