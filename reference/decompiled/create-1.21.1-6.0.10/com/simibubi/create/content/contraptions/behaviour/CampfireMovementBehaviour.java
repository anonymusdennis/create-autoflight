...
public class CampfireMovementBehaviour implements MovementBehaviour {
   ...
   public void tick(MovementContext context) {
      if (context.world != null
         ...
         && (Boolean)context.state.getValue(CampfireBlock.LIT)
         && !context.disabled) {
            ...
            for (int i = 0; i < random.nextInt(2) + 2; i++) {
               context.world
                  .addAlwaysVisibleParticle(
                     ...
                     true,
                     context.position.x() + random.nextDouble() / (random.nextBoolean() ? 3.0 : -3.0),
                     context.position.y() + random.nextDouble() + random.nextDouble(),
                     context.position.z() + random.nextDouble() / (random.nextBoolean() ? 3.0 : -3.0),
                     0.0,
                     0.07,
                     0.0
                  );
