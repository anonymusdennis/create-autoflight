...
public class ElevatorContactScreen extends AbstractSimiScreen {
   ...
   public void init() {
      ...
      this.setFocused(this.shortNameInput);
      ...
      MutableComponent rmbToEdit = CreateLang.translate("gui.schedule.lmb_edit").style(ChatFormatting.DARK_GRAY).style(ChatFormatting.ITALIC).component();
      this.addRenderableOnly(
         new TooltipArea(x + 21, y + 23, 30, 18)
            .withTooltip(ImmutableList.of(CreateLang.translate("elevator_contact.floor_identifier").color(5476833).component(), rmbToEdit))
      );
      this.addRenderableOnly(
         new TooltipArea(x + 57, y + 23, 147, 18)
            .withTooltip(
               ImmutableList.of(
                  CreateLang.translate("elevator_contact.floor_description").color(5476833).component(),
   ...
   protected void renderWindow(GuiGraphics graphics, int mouseX, int mouseY, float partialTicks) {
      ...
      graphics.drawString(
         ...
         (float)(x + (this.background.getWidth() - 8) / 2 - this.font.width(formattedcharsequence) / 2),
         (float)y + 6.0F,
         3094328,
         false
      );
      ((GuiRenderBuilder)GuiGameElement.of(AllBlocks.ELEVATOR_CONTACT.asStack())
   ...
   public boolean mouseClicked(double pMouseX, double pMouseY, int pButton) {
      ...
      if (!consumed
         && pMouseX > (double)(this.guiLeft + 22)
         && pMouseY > (double)(this.guiTop + 24)
         && pMouseX < (double)(this.guiLeft + 50)
         && pMouseY < (double)(this.guiTop + 40)) {
         this.setFocused(this.shortNameInput);
         this.shortNameInput.setFocused(true);
         return true;
      } else {
   ...
   private void confirm() {
      CatnipServices.NETWORK.sendToServer(new ElevatorContactEditPacket(this.pos, this.shortName, this.longName, this.doorControl));
      this.onClose();
   }
}
