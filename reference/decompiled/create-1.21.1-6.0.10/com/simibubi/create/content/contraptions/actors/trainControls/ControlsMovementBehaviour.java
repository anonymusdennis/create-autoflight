      ...
      if (context.temporaryData instanceof ControlsMovementBehaviour.LeverAngles angles) {
         ...
         if (entity instanceof CarriageContraptionEntity cce) {
            ...
            if (ControlsHandler.getContraption() == entity
               ...
               angles.equipAnimation.chase(1.0, 0.2F, Chaser.EXP);
               angles.steering.chase((double)((pressed.contains(3) ? 1 : 0) + (pressed.contains(2) ? -1 : 0)), 0.2F, Chaser.EXP);
               float f = cce.movingBackwards ^ inverted ? -1.0F : 1.0F;
               angles.speed.chase(Math.min(context.motion.length(), 0.5) * (double)f, 0.2F, Chaser.EXP);
            } else {
               angles.equipAnimation.chase(0.0, 0.2F, Chaser.EXP);
               angles.steering.chase(0.0, 0.0, Chaser.EXP);
               angles.speed.chase(0.0, 0.0, Chaser.EXP);
            }

            float pt = AnimationTickHolder.getPartialTicks(context.world);
            ControlsRenderer.render(
               context, renderWorld, matrices, buffer, angles.equipAnimation.getValue(pt), angles.speed.getValue(pt), angles.steering.getValue(pt)
            );
         }
      }
   }
