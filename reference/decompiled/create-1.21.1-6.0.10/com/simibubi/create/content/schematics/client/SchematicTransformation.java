...
public class SchematicTransformation {
   ...
   public void init(BlockPos anchor, StructurePlaceSettings settings, AABB bounds) {
      ...
      this.getScaleFB().chase(0.0, 0.45F, Chaser.EXP).startWithValue((double)frontBack);
      this.getScaleLR().chase(0.0, 0.45F, Chaser.EXP).startWithValue((double)leftRight);
      this.xOrigin = bounds.getXsize() / 2.0;
      this.zOrigin = bounds.getZsize() / 2.0;
      int r = -(settings.getRotation().ordinal() * 90);
      this.rotation.chase(0.0, 0.45F, Chaser.EXP).startWithValue((double)r);
   ...
   public void applyTransformations(PoseStack ms, Vec3 camera) {
      ...
      TransformStack.of(ms).translate(VecHelper.lerp(pt, this.prevChasingPos, this.chasingPos).subtract(camera));
   ...
   public boolean isFlipped() {
      return this.getMirrorModifier(Axis.X) < 0 != this.getMirrorModifier(Axis.Z) < 0;
   }
   ...
   public Vec3 getRotationOffset(boolean ignoreMirrors) {
      ...
      if ((int)(this.zOrigin * 2.0) % 2 != (int)(this.xOrigin * 2.0) % 2) {
         ...
         if (!ignoreMirrors) {
            xIn *= (float)this.getMirrorModifier(Axis.X);
            zIn *= (float)this.getMirrorModifier(Axis.Z);
         }
   ...
   public Vec3 toLocalSpace(Vec3 vec) {
      ...
      vec = VecHelper.rotate(vec, (double)(-this.rotation.getValue(pt)), Axis.Y);
   ...
   public BlockPos getAnchor() {
      ...
      vec = VecHelper.rotate(vec, (double)this.rotation.getChaseTarget(), Axis.Y);
   ...
   public int getMirrorModifier(Axis axis) {
      return axis == Axis.Z ? (int)this.getScaleLR().getChaseTarget() : (int)this.getScaleFB().getChaseTarget();
   }
