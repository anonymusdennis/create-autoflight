...
public abstract class SchematicToolBase implements ISchematicTool {
   ...
   public void updateTargetPos() {
      ...
      if (this.selectIgnoreBlocks) {
         ...
         this.selectedPos = BlockPos.containing(player.getEyePosition(pt).add(player.getLookAngle().scale((double)this.selectionRange)));
         if (snap) {
      ...
      if (this.schematicHandler.isDeployed()) {
         ...
         if (this.renderSelectedFace) {
            outline.getParams()
               ...
               .withFaceTextures(AllSpecialTextures.CHECKERED, AllKeys.ctrlDown() ? AllSpecialTextures.HIGHLIGHT_CHECKERED : AllSpecialTextures.CHECKERED);
         }
