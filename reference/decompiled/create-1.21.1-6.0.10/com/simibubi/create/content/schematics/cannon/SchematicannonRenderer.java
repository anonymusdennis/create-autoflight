...
public class SchematicannonRenderer extends SafeBlockEntityRenderer<SchematicannonBlockEntity> {
      ...
      if (target != null) {
         ...
         if (blockEntity.previousTarget != null) {
            diff = Vec3.atLowerCornerOf(blockEntity.previousTarget)
               .add(Vec3.atLowerCornerOf(target.subtract(blockEntity.previousTarget)).scale((double)partialTicks))
   ...
   private static void renderLaunchedBlocks(
      SchematicannonBlockEntity blockEntity, float partialTicks, PoseStack ms, MultiBufferSource buffer, int light, int overlay
   ) {
      for (LaunchedItem launched : blockEntity.flyingBlocks) {
         if (launched.ticksRemaining != 0) {
            ...
            if (launched.ticksRemaining == launched.totalTicks && blockEntity.firstRenderTick) {
               ...
               for (int i = 0; i < 10; i++) {
                  ...
                  blockEntity.getLevel().addParticle(ParticleTypes.CLOUD, start.x + rX, start.y + rY, start.z + rZ, sX, sY, sZ);
               }
            }
         }
      }
