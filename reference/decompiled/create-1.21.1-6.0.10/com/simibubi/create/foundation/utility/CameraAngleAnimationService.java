...
public class CameraAngleAnimationService {
   ...
   private static float getCurrentYaw() {
      return Minecraft.getInstance().player == null ? 0.0F : Mth.wrapDegrees(Minecraft.getInstance().player.getYRot());
   }

   private static float getCurrentPitch() {
      return Minecraft.getInstance().player == null ? 0.0F : Mth.wrapDegrees(Minecraft.getInstance().player.getXRot());
   }
   ...
   private static void setupChaser(LerpedFloat rotation, float target) {
      if (animationMode == CameraAngleAnimationService.Mode.LINEAR) {
         rotation.chase((double)target, animationSpeed > 0.0F ? (double)animationSpeed : 2.0, Chaser.LINEAR);
      } else if (animationMode == CameraAngleAnimationService.Mode.EXPONENTIAL) {
         rotation.chase((double)target, animationSpeed > 0.0F ? (double)animationSpeed : 0.25, Chaser.EXP);
      }
   }
