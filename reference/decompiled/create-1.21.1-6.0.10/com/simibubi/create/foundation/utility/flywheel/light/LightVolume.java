...
public class LightVolume implements Box {
   ...
   public short getPackedLight(int x, int y, int z) {
      return this.box.contains(x, y, z) ? MemoryUtil.memGetShort(this.levelPosToPtr(x, y, z)) : 0;
   }
   ...
   protected void writeLight(int x, int y, int z, int block, int sky) {
      byte b = (byte)((block & 15) << 4);
      byte s = (byte)((sky & 15) << 4);
      long ptr = this.boxPosToPtr(x, y, z);
      MemoryUtil.memPutByte(ptr, b);
      MemoryUtil.memPutByte(ptr + 1L, s);
   }
   ...
   protected void writeBlock(int x, int y, int z, int block) {
      byte b = (byte)((block & 15) << 4);
      MemoryUtil.memPutByte(this.boxPosToPtr(x, y, z), b);
   }
   ...
   protected void writeSky(int x, int y, int z, int sky) {
      byte s = (byte)((sky & 15) << 4);
      MemoryUtil.memPutByte(this.boxPosToPtr(x, y, z) + 1L, s);
   }
   ...
   protected int levelPosToPtrOffset(int x, int y, int z) {
      ...
      y -= this.box.getMinY();
      z -= this.box.getMinZ();
      return this.boxPosToPtrOffset(x, y, z);
   }
