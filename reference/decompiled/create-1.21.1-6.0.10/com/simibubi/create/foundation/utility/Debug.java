...
public class Debug {
   ...
   public static void debugChatAndShowStack(String message, int depth) {
      if (Minecraft.getInstance().player != null) {
         Minecraft.getInstance().player.displayClientMessage(Component.literal(message).append("@").append(debugStack(depth)), false);
      }
   }
   ...
   public static Component debugStack(int depth) {
      ...
      MutableComponent text = Component.literal("[").append(Component.literal(getLogicalSide()).withStyle(ChatFormatting.GOLD)).append("] ");
      ...
      for (int i = 1; i < depth + 2 && i < stackTraceElements.length; i++) {
         ...
         if (!e.getClassName().equals(Debug.class.getName())) {
            text.append(Component.literal(e.getMethodName()).withStyle(ChatFormatting.YELLOW)).append(", ");
         }
      }
