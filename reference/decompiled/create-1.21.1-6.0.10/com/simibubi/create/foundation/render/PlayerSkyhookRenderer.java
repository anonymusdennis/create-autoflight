...
public class PlayerSkyhookRenderer {
   ...
   public static void afterSetupAnim(Player player, HumanoidModel<?> model) {
      if (hangingPlayers.contains(player.getUUID())) {
         setHangingPose(player.getMainArm() == HumanoidArm.LEFT ^ !AllTags.AllItemTags.CHAIN_RIDEABLE.matches(player.getMainHandItem()), model);
      }
   }
   ...
   private static void setHangingPose(boolean isLeftArmMain, HumanoidModel<?> model) {
      if (!Minecraft.getInstance().isPaused()) {
         ...
         model.leftLeg.resetPose();
         model.rightLeg.resetPose();
         float time = (float)AnimationTickHolder.getTicks(true) + AnimationTickHolder.getPartialTicks();
         float mainCycle = Mth.sin((float)((double)((time + 10.0F) * 0.3F) / Math.PI));
         float limbCycle = Mth.sin((float)((double)(time * 0.3F) / Math.PI));
         ...
         float armPivotX = offsetX * Mth.cos(bodySwing) - offsetY * Mth.sin(bodySwing) + (float)(isLeftArmMain ? -1 : 1) * 4.5F;
         float armPivotY = offsetX * Mth.sin(bodySwing) + offsetY * Mth.cos(bodySwing) + 2.0F;
         hangingArm.xRot = -AngleHelper.rad(150.0);
         hangingArm.zRot = (float)(isLeftArmMain ? -1 : 1) * AngleHelper.rad(15.0);
         offsetX = otherArm.x;
         offsetY = otherArm.y;
         ...
         otherArm.zRot = (float)(isLeftArmMain ? -1 : 1) * -AngleHelper.rad(20.0) + 0.5F * bodySwing + limbSwing;
         ...
         leadingLeg.xRot = -AngleHelper.rad(25.0);
         leadingLeg.zRot = (float)(isLeftArmMain ? -1 : 1) * AngleHelper.rad(10.0) + 0.5F * bodySwing + limbSwing;
         trailingLeg.y -= 0.8F;
         ...
         trailingLeg.xRot = AngleHelper.rad(10.0);
         trailingLeg.zRot = (float)(isLeftArmMain ? -1 : 1) * -AngleHelper.rad(10.0) + 0.5F * bodySwing + limbSwing;
