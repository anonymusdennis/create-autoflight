...
public abstract class DirectionalAxisBlockStateGen extends SpecialBlockStateGen {
   ...
   protected int getXRotation(BlockState state) {
      Direction direction = (Direction)state.getValue(GaugeBlock.FACING);
      boolean alongFirst = (Boolean)state.getValue(GaugeBlock.AXIS_ALONG_FIRST_COORDINATE);
      if (direction == Direction.DOWN) {
         return 180;
      } else if (direction == Direction.UP) {
         return 0;
      } else {
         return direction.getAxis() == Axis.X == alongFirst ? 90 : 0;
      }
   }
   ...
   public <T extends Block> ModelFile getModel(DataGenContext<Block, T> ctx, RegistrateBlockstateProvider prov, BlockState state) {
      boolean vertical = ((Direction)state.getValue(GaugeBlock.FACING)).getAxis().isVertical();
