...
public class LogStrippingFakeRecipes {
   public static List<RecipeHolder<ManualApplicationRecipe>> createRecipes() {
      ...
      if (!(Boolean)AllConfigs.server().recipes.displayLogStrippingRecipes.get()) {
         return recipes;
      } else {
         ItemStack axe = new ItemStack(Items.IRON_AXE);
         axe.set(DataComponents.ATTRIBUTE_MODIFIERS, ItemAttributeModifiers.EMPTY);
         axe.set(DataComponents.CUSTOM_NAME, CreateLang.translateDirect("recipe.item_application.any_axe").withStyle(style -> style.withItalic(false)));
         BuiltInRegistries.ITEM.getTagOrEmpty(ItemTags.LOGS).forEach(stack -> process((Item)stack.value(), recipes, axe));
         return recipes;
      }
   }
   ...
   private static RecipeHolder<ManualApplicationRecipe> create(Item fromItem, Item toItem, ItemStack axe) {
      ...
      ManualApplicationRecipe recipe = new ItemApplicationRecipe.Builder<>(ManualApplicationRecipe::new, id)
         .require(fromItem)
         .require(Ingredient.of(new ItemStack[]{axe}))
         .output(toItem)
         .build();
