...
public class SmartFluidTankBehaviour extends BlockEntityBehaviour {
   ...
   public SmartFluidTankBehaviour(BehaviourType<SmartFluidTankBehaviour> type, SmartBlockEntity be, int tanks, int tankCapacity, boolean enforceVariety) {
      ...
      this.capability = new SmartFluidTankBehaviour.InternalFluidHandler(handlers, enforceVariety);
   ...
   public void write(CompoundTag nbt, Provider registries, boolean clientPacket) {
      ...
      this.forEach(ts -> tanksNBT.add(ts.writeNBT(registries)));
   ...
   public void read(CompoundTag nbt, Provider registries, boolean clientPacket) {
      ...
      NBTHelper.iterateCompoundList(nbt.getList(this.getType().getName() + "Tanks", 10), c -> {
         if (index.intValue() < this.tanks.length) {
            this.tanks[index.intValue()].readNBT(c, registries, clientPacket);
            index.increment();
         }
      });
   }
   ...
   public class TankSegment {
      ...
      public TankSegment(int capacity) {
         ...
         this.fluidLevel = LerpedFloat.linear().startWithValue(0.0).chase(0.0, 0.25, Chaser.EXP);
         this.renderedFluid = FluidStack.EMPTY;
      }

      public void onFluidStackChanged() {
         if (SmartFluidTankBehaviour.this.blockEntity.hasLevel()) {
            this.fluidLevel.chase((double)((float)this.tank.getFluidAmount() / (float)this.tank.getCapacity()), 0.25, Chaser.EXP);
      ...
      public CompoundTag writeNBT(Provider registries) {
         ...
         compound.put("TankContent", this.tank.writeToNBT(registries, new CompoundTag()));
         compound.put("Level", this.fluidLevel.writeNBT());
         return compound;
      }

      public void readNBT(CompoundTag compound, Provider registries, boolean clientPacket) {
