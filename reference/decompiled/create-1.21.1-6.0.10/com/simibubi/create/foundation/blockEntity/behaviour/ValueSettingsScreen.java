...
public class ValueSettingsScreen extends AbstractSimiScreen {
   ...
   protected void init() {
      ...
      for (Component component : this.board.rows()) {
         this.maxLabelWidth = Math.max(this.maxLabelWidth, this.font.width(component));
      }
   ...
   private void setCursor(Vec2 coordinateOfValue) {
      ...
      GLFW.glfwSetCursorPos(this.minecraft.getWindow().getWindow(), (double)coordinateOfValue.x * guiScale, (double)coordinateOfValue.y * guiScale);
   }
   ...
   protected void renderWindow(GuiGraphics graphics, int mouseX, int mouseY, float partialTicks) {
      ...
      if (this.iconMode) {
         for (int i = 0; i <= this.board.maxValue(); i++) {
            fattestLabel = Math.max(fattestLabel, this.font.width(this.board.formatter().format(new ValueSettingsBehaviour.ValueSettings(0, i))));
         }
      }
      ...
      UIRenderHelper.drawStretched(
         graphics, x - 11 + fadeInStart, y - 17, fadeInWidth, this.windowHeight + additionalHeight, zLevel, AllGuiTextures.VALUE_SETTINGS_OUTER_BG
      );
      ...
      UIRenderHelper.drawStretched(
         graphics, x - 10 + fadeInStart, y - 17 + this.windowHeight + additionalHeight, zLevel, fadeInWidth - 2, 1, AllGuiTextures.VALUE_SETTINGS_OUTER_BG
      );
      ...
      UIRenderHelper.drawStretched(
         graphics, x + this.maxLabelWidth + 17, y, this.valueBarWidth + 2, this.board.rows().size() * 11 - 1, zLevel, AllGuiTextures.VALUE_SETTINGS_BAR_BG
      );
      int originalY = y;
         ...
         if (!this.iconMode) {
            ...
            for (int w = 0; w < this.valueBarWidth; w += AllGuiTextures.VALUE_SETTINGS_BAR.getWidth() - 1) {
               UIRenderHelper.drawCropped(
                  ...
                  valueBarX + w,
                  y + 1,
                  Math.min(AllGuiTextures.VALUE_SETTINGS_BAR.getWidth() - 1, this.valueBarWidth - w),
                  8,
                  zLevel,
            ...
            graphics.drawString(this.font, component, x, y + 1, 4464640, false);
         }
      ...
      if (this.ticksOpen >= 1) {
         ...
         if (!closest.equals(this.lastHovered)) {
            ...
            if (this.soundCoolDown == 0) {
               ...
               this.minecraft.getSoundManager().play(SimpleSoundInstance.forUI(AllSoundEvents.SCROLL_VALUE.getMainEvent(), pitch, 0.25F));
         ...
         int cursorWidth = (cursorIcon != null ? 16 : this.font.width(cursorText)) / 2 * 2 + 3;
         ...
         if (cursorIcon != null) {
            ...
            if (fadeInWidth > fattestLabel) {
               graphics.drawString(
                  this.font,
                  cursorText,
                  x - 11 - fatTipOffset + (bgWidth - this.font.width(cursorText)) / 2,
                  originalY + this.windowHeight + additionalHeight - 40,
                  16505981,
                  false
               );
            }
            ...
            graphics.drawString(this.font, cursorText, cursorX + 2, cursorY + 3, 4464640, false);
         }
      }
   }
   ...
   public boolean mouseScrolled(double pMouseX, double pMouseY, double pScrollX, double pScrollY) {
      ...
      int column = closest.value() + (int)Math.signum(pScrollY) * (hasShiftDown() ? this.board.milestoneInterval() : 1);
   ...
   protected void saveAndClose(double pMouseX, double pMouseY) {
      ...
      CatnipServices.NETWORK
         .sendToServer(new ValueSettingsPacket(this.pos, closest.row(), closest.value(), null, null, Direction.UP, AllKeys.ctrlDown(), this.netId));
      this.onClose();
   }
