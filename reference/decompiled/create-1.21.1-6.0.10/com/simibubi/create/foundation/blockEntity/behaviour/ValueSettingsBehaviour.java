...
public interface ValueSettingsBehaviour extends ClipboardCloneable {
   ...
   @Override
   default boolean writeToClipboard(@NotNull Provider registries, CompoundTag tag, Direction side) {
   ...
   @Override
   default boolean readFromClipboard(@NotNull Provider registries, CompoundTag tag, Player player, Direction side, boolean simulate) {
      if (!this.acceptsValueSettings()) {
         return false;
   ...
   public static record ValueSettings(int row, int value) {
      public MutableComponent format() {
         return CreateLang.number((double)this.value).component();
      }
   }
}
