      ...
      if (BlockHelper.hasBlockSolidSide(world.getBlockState(pos.relative(face)), world, pos.relative(face), face.getOpposite())) {
         ...
         for (Direction direction : Iterate.directions) {
            if (direction.getAxis() != face.getAxis()) {
               ...
               if (!BlockHelper.hasBlockSolidSide(world.getBlockState(neighbourPos.relative(face)), world, neighbourPos.relative(face), face.getOpposite())
                  && behaviour.connectivityPredicate.test(world, pos, face, direction)) {
                  sides.add(direction);
               }
            }
         }
