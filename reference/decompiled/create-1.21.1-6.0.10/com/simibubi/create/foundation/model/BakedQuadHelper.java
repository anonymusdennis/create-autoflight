...
public final class BakedQuadHelper {
   ...
   public static BakedQuad clone(BakedQuad quad) {
      return new BakedQuad(
         Arrays.copyOf(quad.getVertices(), quad.getVertices().length), quad.getTintIndex(), quad.getDirection(), quad.getSprite(), quad.isShade()
      );
   }
   ...
   public static void setNormalXYZ(int[] vertexData, int vertex, Vec3 xyz) {
      int x = Byte.toUnsignedInt((byte)((int)(Mth.clamp(xyz.x, -1.0, 1.0) * 127.0)));
      int y = Byte.toUnsignedInt((byte)((int)(Mth.clamp(xyz.y, -1.0, 1.0) * 127.0)));
      int z = Byte.toUnsignedInt((byte)((int)(Mth.clamp(xyz.z, -1.0, 1.0) * 127.0)));
      int data = x << 24 | y << 16 | z << 8;
      vertexData[vertex * VERTEX_STRIDE + 7] = data;
   }
