...
public class BakedModelHelper {
   public static int[] cropAndMove(int[] vertexData, TextureAtlasSprite sprite, AABB crop, Vec3 move) {
      ...
      float uScale = (float)Math.round(
         (double)(SpriteShiftEntry.getUnInterpolatedU(sprite, u3) - SpriteShiftEntry.getUnInterpolatedU(sprite, u0)) / xyz3.distanceTo(xyz0)
      );
      float vScale = (float)Math.round(
         (double)(SpriteShiftEntry.getUnInterpolatedV(sprite, v1) - SpriteShiftEntry.getUnInterpolatedV(sprite, v0)) / xyz1.distanceTo(xyz0)
      );
      if (uScale == 0.0F) {
         ...
         uScale = (float)Math.round(
            (double)(SpriteShiftEntry.getUnInterpolatedU(sprite, u1) - SpriteShiftEntry.getUnInterpolatedU(sprite, u0)) / xyz1.distanceTo(xyz0)
         );
         vScale = (float)Math.round(
            (double)(SpriteShiftEntry.getUnInterpolatedV(sprite, v3) - SpriteShiftEntry.getUnInterpolatedV(sprite, v0)) / xyz3.distanceTo(xyz0)
         );
      }
      ...
      for (int i = 0; i < size; i++) {
         ...
         if (newSprite != null && sprite != newSprite) {
            ...
            for (int vertex = 0; vertex < 4; vertex++) {
               float u = BakedQuadHelper.getU(vertexData, vertex);
               float v = BakedQuadHelper.getV(vertexData, vertex);
               BakedQuadHelper.setU(vertexData, vertex, newSprite.getU(SpriteShiftEntry.getUnInterpolatedU(sprite, u)));
               BakedQuadHelper.setV(vertexData, vertex, newSprite.getV(SpriteShiftEntry.getUnInterpolatedV(sprite, v)));
            }
