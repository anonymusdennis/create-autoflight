...
public record ItemDescription(ImmutableList<Component> lines, ImmutableList<Component> linesOnShift, ImmutableList<Component> linesOnCtrl) {
   ...
   public static void fillBuilder(ItemDescription.Builder builder, String translationKey) {
      ...
      if (I18n.exists(summaryKey)) {
         builder.addSummary(I18n.get(summaryKey, new Object[0]));
      }
      ...
      for (int i = 1; i < 100; i++) {
         ...
         builder.addBehaviour(I18n.get(conditionKey, new Object[0]), I18n.get(behaviourKey, new Object[0]));
      }
      ...
      for (int i = 1; i < 100; i++) {
         ...
         builder.addAction(I18n.get(controlKey, new Object[0]), I18n.get(actionKey, new Object[0]));
      }
   }
   ...
   public static String getTooltipTranslationKey(Item item) {
      return CUSTOM_TOOLTIP_KEYS.containsKey(item) ? CUSTOM_TOOLTIP_KEYS.get(item).get() + ".tooltip" : item.getDescriptionId() + ".tooltip";
   }
   ...
   public static class Builder {
      ...
      public ItemDescription build() {
         ...
         for (Pair<String, String> behaviourPair : this.behaviours) {
            ...
            linesOnShift.add(Component.literal(condition).withStyle(ChatFormatting.GRAY));
         ...
         for (Pair<String, String> actionPair : this.actions) {
            ...
            linesOnCtrl.add(Component.literal(condition).withStyle(ChatFormatting.GRAY));
            ...
            for (List<Component> list : Arrays.asList(lines, linesOnShift, linesOnCtrl)) {
               ...
               if (holdDesc.length == 2 && holdCtrl.length == 2) {
                  if (hasControls) {
                     ...
                     tabBuilder.append(Component.literal(holdCtrl[0]).withStyle(ChatFormatting.DARK_GRAY));
                     tabBuilder.append(keyCtrl.plainCopy().withStyle(ctrl ? ChatFormatting.WHITE : ChatFormatting.GRAY));
                     tabBuilder.append(Component.literal(holdCtrl[1]).withStyle(ChatFormatting.DARK_GRAY));
                  ...
                  if (hasDescription) {
                     ...
                     tabBuilder.append(keyShift.plainCopy().withStyle(shift ? ChatFormatting.WHITE : ChatFormatting.GRAY));
