...
public class SoundScapes {
   ...
   private static SoundScape crushing(float pitch, SoundScapes.AmbienceGroup group) {
      return new SoundScape(pitch, group)
         .repeating(AllSoundEvents.CRUSHING_1.getMainEvent(), 1.545F, 0.75F, 1)
   ...
   public static void play(SoundScapes.AmbienceGroup group, BlockPos pos, float pitch) {
      if ((Boolean)AllConfigs.client().enableAmbientSounds.get()) {
         if (!outOfRange(pos)) {
   ...
   public static void tick() {
      ...
      if (AnimationTickHolder.getTicks() % 5 == 0) {
         boolean disable = !(Boolean)AllConfigs.client().enableAmbientSounds.get();
         ...
         while (iterator.hasNext()) {
            Entry<Pair<SoundScapes.AmbienceGroup, SoundScapes.PitchGroup>, SoundScape> entry = iterator.next();
            Pair<SoundScapes.AmbienceGroup, SoundScapes.PitchGroup> key = entry.getKey();
            SoundScape value = entry.getValue();
            if (disable || getSoundCount((SoundScapes.AmbienceGroup)key.getFirst(), (SoundScapes.PitchGroup)key.getSecond()) == 0) {
               value.remove();
               iterator.remove();
            }
         }
   ...
   protected static boolean outOfRange(BlockPos pos) {
      return !getCameraPos().closerThan(pos, 16.0);
   }
   ...
   public static enum AmbienceGroup {
      ...
      public SoundScape instantiate(float pitch) {
         return this.factory.apply(pitch, this);
      }
   }
