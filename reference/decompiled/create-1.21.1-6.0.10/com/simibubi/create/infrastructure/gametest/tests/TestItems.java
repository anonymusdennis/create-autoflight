...
public class TestItems {
   @GameTest(
      template = "andesite_tunnel_split"
   )
   public static void andesiteTunnelSplit(CreateGameTestHelper helper) {
      ...
      Map<BlockPos, ItemStack> outputs = Map.of(
         new BlockPos(2, 2, 1),
         new ItemStack((ItemLike)AllItems.BRASS_INGOT.get(), 1),
         new BlockPos(3, 2, 1),
         new ItemStack((ItemLike)AllItems.BRASS_INGOT.get(), 1),
         new BlockPos(4, 2, 2),
         new ItemStack((ItemLike)AllItems.BRASS_INGOT.get(), 3)
      );
      helper.succeedWhen(() -> outputs.forEach(helper::assertContainerContains));
   }

   @GameTest(
      template = "arm_multi_output",
      timeoutTicks = 200
   )
         ...
         () -> {
            for (BlockPos pos : blazeBurners) {
               helper.assertBlockState(
                  pos, state -> state.getValue(BlazeBurnerBlock.HEAT_LEVEL) == BlazeBurnerBlock.HeatLevel.KINDLED, () -> "Blaze burner isn't lit!"
               );
            }
         }
      );
   ...
   public static void attributeFilters(CreateGameTestHelper helper) {
      ...
      Map<BlockPos, ItemStack> outputs = Map.of(
         new BlockPos(3, 2, 1),
         new ItemStack((ItemLike)AllBlocks.BRASS_BLOCK.get()),
         new BlockPos(4, 2, 1),
         new ItemStack(Items.APPLE),
         new BlockPos(5, 2, 1),
         new ItemStack(Items.WATER_BUCKET),
         new BlockPos(6, 2, 1),
         EnchantedBookItem.createForEnchantment(new EnchantmentInstance(PROTECTION_ENCHANT, 1)),
         new BlockPos(7, 2, 1),
         (ItemStack)Util.make(new ItemStack(Items.NETHERITE_SWORD), s -> s.setDamageValue(1)),
         new BlockPos(8, 2, 1),
   ...
   @GameTest(
      template = "brass_tunnel_filtering"
   )
   public static void brassTunnelFiltering(CreateGameTestHelper helper) {
      Map<BlockPos, ItemStack> outputs = Map.of(
         ...
         new BlockPos(4, 2, 3),
         new ItemStack((ItemLike)AllItems.ZINC_INGOT.get(), 4),
         new BlockPos(4, 2, 4),
   ...
   public static void brassTunnelPreferNearest(CreateGameTestHelper helper) {
      ...
      helper.succeedWhen(() -> out.forEach(pos -> helper.assertContainerContains(pos, (ItemLike)AllBlocks.BRASS_CASING.get())));
   }
   ...
   @GameTest(
      template = "brass_tunnel_sync_input",
      timeoutTicks = 200
   )
   ...
   @GameTest(
      template = "smart_observer_chutes"
   )
   ...
   @GameTest(
      template = "smart_observer_counting"
   )
   public static void smartObserverCounting(CreateGameTestHelper helper) {
      ...
      NixieTubeBlockEntity doubleChestNixie = helper.getBlockEntity(
         (BlockEntityType<NixieTubeBlockEntity>)AllBlockEntityTypes.NIXIE_TUBE.get(), doubleChestNixiePos
      );
      helper.succeedWhen(() -> {
   ...
   @GameTest(
      template = "smart_observer_filtered_storage"
   )
   ...
   @GameTest(
      template = "smart_observer_storage"
   )
   ...
   public static void depotDisplay(CreateGameTestHelper helper) {
      ...
      List<DepotBlockEntity> depots = Stream.of(new BlockPos(2, 2, 1), new BlockPos(1, 2, 1))
         .map(pos -> helper.getBlockEntity((BlockEntityType<? extends DepotBlockEntity>)AllBlockEntityTypes.DEPOT.get(), pos))
         .toList();
      List<BlockPos> levers = List.of(new BlockPos(2, 5, 0), new BlockPos(1, 5, 0));
      levers.forEach(helper::pullLever);
      FlapDisplayBlockEntity display = helper.<FlapDisplayBlockEntity>getBlockEntity(
            (BlockEntityType<FlapDisplayBlockEntity>)AllBlockEntityTypes.FLAP_DISPLAY.get(), displayPos
         )
         .getController();
      helper.succeedWhen(() -> {
         for (int i = 0; i < 2; i++) {
   ...
   @GameTest(
      template = "vault_comparator_output"
   )
   ...
   @GameTest(
      template = "depot_comparator_output"
   )
   ...
   @GameTest(
      template = "fan_processing",
      timeoutTicks = 200
   )
   public static void fanProcessing(CreateGameTestHelper helper) {
      BlockPos.betweenClosed(new BlockPos(2, 7, 3), new BlockPos(11, 7, 3)).forEach(pos -> helper.setBlock(pos, Blocks.REDSTONE_WIRE));
