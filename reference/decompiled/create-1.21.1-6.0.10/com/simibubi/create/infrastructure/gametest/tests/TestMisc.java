...
public class TestMisc {
   ...
   public static void schematicannon(CreateGameTestHelper helper) {
      ...
      SchematicExport.saveSchematic(CreatePaths.UPLOADED_SCHEMATICS_DIR.resolve("Deployer"), "schematicannon_gametest", true, level, whiteEndBottom, redEndTop);
      ItemStack schematic = SchematicItem.create(level, "schematicannon_gametest.nbt", "Deployer");
   ...
   @GameTest(
      template = "smart_observer_blocks"
   )
   ...
   @GameTest(
      template = "threshold_switch_pulley"
   )
      ...
      helper.succeedWhen(
         () -> {
            ThresholdSwitchBlockEntity switchBe = helper.getBlockEntity(
               (BlockEntityType<ThresholdSwitchBlockEntity>)AllBlockEntityTypes.THRESHOLD_SWITCH.get(), switchPos
            );
