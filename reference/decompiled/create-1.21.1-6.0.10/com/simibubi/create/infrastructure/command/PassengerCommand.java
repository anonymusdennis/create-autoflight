...
public class PassengerCommand {
   static ArgumentBuilder<CommandSourceStack, ?> register() {
      return ((LiteralArgumentBuilder)Commands.literal("passenger").requires(cs -> cs.hasPermission(2)))
         .then(
            Commands.argument("rider", EntityArgument.entity())
               .then(
                  ((RequiredArgumentBuilder)Commands.argument("vehicle", EntityArgument.entity()).executes(ctx -> {
                        run((CommandSourceStack)ctx.getSource(), EntityArgument.getEntity(ctx, "vehicle"), EntityArgument.getEntity(ctx, "rider"), 0);
                        return 1;
                     }))
                     .then(
                        Commands.argument("seatIndex", IntegerArgumentType.integer(0))
                           .executes(
                              ctx -> {
                                 run(
