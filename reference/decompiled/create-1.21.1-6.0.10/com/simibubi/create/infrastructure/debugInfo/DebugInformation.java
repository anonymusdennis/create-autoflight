...
public class DebugInformation {
   ...
   private static final ImmutableMap<String, String> mcSystemInfo = (ImmutableMap<String, String>)Util.make(() -> {
   ...
   public static String getIndent(int depth) {
      return Stream.<CharSequence>generate(() -> "\t").limit((long)depth).collect(Collectors.joining());
   }
   ...
   static {
      DebugInfoSection.builder("Create")
      ...
      CatnipServices.PLATFORM
         .executeOnClientOnly(
            () -> () -> DebugInfoSection.builder("Graphics")
                     .put("Flywheel Version", ModList.get().getModContainerById("flywheel").map(c -> c.getModInfo().getVersion().toString()).orElse("None"))
                     ...
                     .put("Graphics Mode", () -> ((GraphicsStatus)Minecraft.getInstance().options.graphicsMode().get()).name().toLowerCase(Locale.ROOT))
