...
public class DeployerScenes {
   public static void filter(SceneBuilder builder, SceneBuildingUtil util) {
      ...
      scene.world()
         .modifyBlockEntityNBT(deployerSelection, teType, nbt -> nbt.put("HeldItem", ItemStack.EMPTY.saveOptional(scene.world().getHolderLookupProvider())));
      scene.world().moveDeployer(deployerPos, -1.0F, 25);
      scene.idle(20);
      ...
      scene.effects()
         .emitParticles(
            util.vector().topOf(deployerPos.west(2)).add(0.0, -0.25, 0.0),
            scene.effects()
               .particleEmitterWithinBlockSpace(
                  new BlockParticleOption(ParticleTypes.BLOCK, Blocks.PINK_WOOL.defaultBlockState()), util.vector().of(0.0, 0.0, 0.0)
               ),
            25.0F,
            1
         );
      ...
      scene.world()
         .createItemEntity(
            util.vector().centerOf(deployerPos.north()).subtract(0.0, 0.45, 0.0), util.vector().of(0.0, 0.0, -0.1), new ItemStack(Items.PINK_WOOL)
         );
   ...
   public static void modes(SceneBuilder builder, SceneBuildingUtil util) {
      ...
      scene.world()
         .modifyBlockEntityNBT(
            deployerSelection, DeployerBlockEntity.class, nbt -> nbt.put("HeldItem", tool.saveOptional(scene.world().getHolderLookupProvider()))
         );
      scene.idle(45);
   ...
   public static void processing(SceneBuilder builder, SceneBuildingUtil util) {
      ...
      scene.idle(7);
      scene.world()
         .modifyBlockEntityNBT(pressS, DeployerBlockEntity.class, nbt -> nbt.put("HeldItem", tool.saveOptional(scene.world().getHolderLookupProvider())));
      scene.idle(25);
