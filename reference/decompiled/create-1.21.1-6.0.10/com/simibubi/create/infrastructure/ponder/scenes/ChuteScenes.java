...
public class ChuteScenes {
   public static void downward(SceneBuilder builder, SceneBuildingUtil util) {
      ...
      ElementLink<EntityElement> remove = scene.world()
         .createItemEntity(util.vector().centerOf(util.grid().at(1, 5, 2)), util.vector().of(0.0, 0.1, 0.0), stack);
      scene.idle(15);
      ...
      scene.idle(7);
      scene.world().modifyBlock(util.grid().at(3, 3, 3), s -> (BlockState)s.setValue(ChuteBlock.SHAPE, ChuteBlock.Shape.WINDOW), false);
      scene.overlay()
         .showText(50)
         .attachKeyFrame()
      ...
      scene.idle(7);
      scene.world().modifyBlock(util.grid().at(3, 2, 3), s -> (BlockState)s.setValue(ChuteBlock.SHAPE, ChuteBlock.Shape.ENCASED), false);
      scene.overlay()
         .showText(50)
      ...
      scene.world().modifyBlock(util.grid().at(2, 2, 1), s -> (BlockState)s.setValue(ChuteBlock.SHAPE, ChuteBlock.Shape.NORMAL), false);
      scene.world().modifyBlock(util.grid().at(2, 3, 2), s -> (BlockState)s.setValue(ChuteBlock.SHAPE, ChuteBlock.Shape.INTERSECTION), false);
      ...
      for (int i = 0; i < 3; i++) {
         remove = scene.world()
            .createItemEntity(
               ...
               util.vector().of(0.0, 0.1, 0.0).add(Vec3.atLowerCornerOf(offset.getNormal()).scale(-0.1)),
               stack
            );
         scene.idle(12);
   ...
   public static void upward(SceneBuilder builder, SceneBuildingUtil util) {
      ...
      scene.overlay()
         ...
         .attachKeyFrame()
         .pointAt(util.vector().centerOf(util.grid().at(3, 1, 2)).add(0.0, 0.1875, 0.0))
         .placeNearTarget();
      scene.idle(32);
   ...
   public static void smart(SceneBuilder builder, SceneBuildingUtil util) {
      ...
      scene.overlay()
         ...
         .attachKeyFrame()
         .pointAt(filter.add(0.0, 0.0, 0.125))
         .placeNearTarget();
      scene.idle(60);
