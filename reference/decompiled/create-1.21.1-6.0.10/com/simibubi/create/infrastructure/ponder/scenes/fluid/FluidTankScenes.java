...
public class FluidTankScenes {
   public static void storage(SceneBuilder builder, SceneBuildingUtil util) {
      ...
      scene.idle(5);
      FluidStack content = new FluidStack(((Flowing)AllFluids.CHOCOLATE.get()).getSource(), 16000);
      scene.world().modifyBlockEntity(tankPos, FluidTankBlockEntity.class, be -> be.getTankInventory().fill(content, FluidAction.EXECUTE));
      scene.idle(25);
      ...
      scene.world().modifyBlock(pumpPos, s -> (BlockState)s.setValue(PumpBlock.FACING, Direction.SOUTH), false);
      scene.idle(10);
      ...
      scene.world().modifyBlockEntity(util.grid().at(2, 0, 5), FluidTankBlockEntity.class, be -> be.getTankInventory().fill(content, FluidAction.EXECUTE));
      scene.idle(20);

      for (int i = 0; i < 4; i++) {
         scene.world().modifyBlockEntity(tankPos, FluidTankBlockEntity.class, be -> be.getTankInventory().drain(2000, FluidAction.EXECUTE));
         scene.idle(5);
      }
      ...
      scene.idle(40);
      scene.world().modifyBlock(pumpPos, s -> (BlockState)s.setValue(PumpBlock.FACING, Direction.NORTH), true);
      scene.world().propagatePipeChange(pumpPos);

      for (int i = 0; i < 4; i++) {
         scene.world()
            .modifyBlockEntity(
               tankPos, FluidTankBlockEntity.class, be -> be.getTankInventory().fill(FluidHelper.copyStackWithAmount(content, 2000), FluidAction.EXECUTE)
            );
         scene.idle(5);
      }
      ...
      scene.overlay()
         ...
         .attachKeyFrame()
         .placeNearTarget()
         .pointAt(util.vector().blockSurface(util.grid().at(2, 1, 1), Direction.DOWN).add(0.0, 0.125, 0.0));
      scene.idle(50);
      ...
      scene.world().modifyBlockEntity(util.grid().at(4, 3, 0), SpoutBlockEntity.class, be -> {
         IFluidHandler handler = (IFluidHandler)be.getLevel().getCapability(FluidHandler.BLOCK, be.getBlockPos(), null);
         if (handler != null) {
      ...
      scene.idle(90);
      ItemStack chocBucket = ((Flowing)AllFluids.CHOCOLATE.get())
         .getFluidType()
         .getBucket(new FluidStack(FluidHelper.convertToStill((Fluid)AllFluids.CHOCOLATE.get()), 1000));
      ...
      for (int i = 0; i < 10; i++) {
         scene.effects()
            .emitParticles(
               util.vector().topOf(3, 1, 1).add(0.0, 0.0625, 0.0),
               scene.effects().simpleParticleEmitter(FluidFX.getFluidParticle(content), VecHelper.offsetRandomly(Vec3.ZERO, random, 0.1F)),
               1.0F,
               1
            );
      }
   ...
   public static void sizes(SceneBuilder scene, SceneBuildingUtil util) {
      ...
      scene.idle(7);
      scene.world().modifyBlocks(full2, s -> (BlockState)s.setValue(FluidTankBlock.SHAPE, FluidTankBlock.Shape.PLAIN), false);
      scene.idle(30);
   ...
   public static void creative(SceneBuilder builder, SceneBuildingUtil util) {
      ...
      scene.world()
         .modifyBlockEntity(
            ...
            be -> ((CreativeFluidTankBlockEntity.CreativeSmartFluidTank)be.getTankInventory()).setContainedFluid(new FluidStack(Fluids.FLOWING_LAVA, 1000))
         );
      scene.idle(5);
      scene.overlay()
         .showText(50)
