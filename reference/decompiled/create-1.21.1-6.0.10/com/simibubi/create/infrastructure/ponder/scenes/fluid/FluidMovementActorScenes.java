...
public class FluidMovementActorScenes {
   public static void transfer(SceneBuilder builder, SceneBuildingUtil util) {
      ...
      scene.world().modifyBlock(pumpPos, s -> (BlockState)s.setValue(PumpBlock.FACING, Direction.NORTH), false);
      ...
      scene.overlay()
         .showText(40)
         .placeNearTarget()
         .pointAt(util.vector().of(3.0, 3.0, 2.5))
      ...
      for (int i = 0; i < 16; i++) {
         if (i == 8) {
            scene.overlay()
               .showControls(util.vector().blockSurface(util.grid().at(5, 3, 2), Direction.WEST).add(0.0, 0.5, 0.0), Pointing.LEFT, 30)
               .withItem(bucket);
         }
      ...
      scene.world().modifyBlock(pumpPos, s -> (BlockState)s.setValue(PumpBlock.FACING, Direction.SOUTH), true);
