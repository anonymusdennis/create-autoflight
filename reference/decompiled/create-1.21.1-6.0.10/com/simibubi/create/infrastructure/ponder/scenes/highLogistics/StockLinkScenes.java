...
public class StockLinkScenes {
   public static void stockLink(SceneBuilder builder, SceneBuildingUtil util) {
      ...
      scene.idle(20);
      scene.overlay().chaseBoundingBoxOutline(PonderPalette.GREEN, link1, bb1.move(util.vector().of(2.0, 0.0, 0.0)), 40);
      scene.overlay().chaseBoundingBoxOutline(PonderPalette.GREEN, link2, bb1, 40);
      scene.overlay()
         .showLine(
            PonderPalette.GREEN,
            util.vector().centerOf(link1.below()).subtract(0.0, 0.25, 0.0),
            util.vector().centerOf(link2.below()).subtract(0.0, 0.25, 0.0),
            40
         );
      scene.idle(60);
      ...
      scene.overlay()
         ...
         .attachKeyFrame()
         .placeNearTarget()
         .pointAt(util.vector().of(2.0, 1.5, 3.0));
      scene.idle(40);
      ...
      scene.overlay()
         ...
         .placeNearTarget()
         .pointAt(util.vector().centerOf(util.grid().at(2, 2, 3)).add(-0.25, 0.0, 0.0));
      scene.idle(70);
      scene.world().toggleRedstonePower(util.select().fromTo(1, 2, 2, 2, 2, 3));
      scene.idle(10);
