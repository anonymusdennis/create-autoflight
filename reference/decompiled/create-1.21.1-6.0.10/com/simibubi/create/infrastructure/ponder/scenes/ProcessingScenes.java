...
public class ProcessingScenes {
   public static void millstone(SceneBuilder builder, SceneBuildingUtil util) {
      ...
      scene.overlay()
         ...
         .pointAt(util.vector().blockSurface(millstone, Direction.WEST).add(-0.5, 0.4, 0.0))
         .placeNearTarget();
      scene.idle(60);
   }
   ...
   public static void crushingWheels(SceneBuilder builder, SceneBuildingUtil util) {
      ...
      ParticleEmitter blockSpace = scene.effects()
         .particleEmitterWithinBlockSpace(new ItemParticleOption(ParticleTypes.ITEM, input), util.vector().of(0.0, 0.0, 0.0));
      ...
      scene.overlay()
         ...
         .pointAt(centerTop.add(0.0, 0.5, 0.0))
         .placeNearTarget();
      scene.idle(40);
   ...
   public static void pressing(SceneBuilder builder, SceneBuildingUtil util) {
      ...
      scene.overlay()
         .showText(60)
         .pointAt(pressSide.subtract(0.0, 2.0, 0.0))
         .placeNearTarget()
   ...
   public static void mixing(SceneBuilder builder, SceneBuildingUtil util) {
      ...
      scene.world()
         .modifyBlockEntityNBT(
            util.select().position(basin),
            BasinBlockEntity.class,
            nbt -> nbt.put(
                  "VisualizedItems",
                  NBTHelper.writeCompoundList(
                     ImmutableList.of(IntAttached.with(1, purple)),
                     ia -> (CompoundTag)((ItemStack)ia.getValue()).saveOptional(scene.world().getHolderLookupProvider())
                  )
               )
         );
      scene.idle(4);
      ...
      scene.world()
         .setBlock(
            util.grid().at(1, 1, 2),
            (BlockState)AllBlocks.BLAZE_BURNER.getDefaultState().setValue(BlazeBurnerBlock.HEAT_LEVEL, BlazeBurnerBlock.HeatLevel.KINDLED),
            true
         );
      scene.idle(10);
      scene.overlay()
         .showText(80)
         .pointAt(basinSide.subtract(0.0, 1.0, 0.0))
         .placeNearTarget()
   ...
   public static void emptyBlazeBurner(SceneBuilder scene, SceneBuildingUtil util) {
      ...
      scene.world().createEntity(w -> {
         ...
         blazeEntity.setYRot(blazeEntity.yRotO = 180.0F);
         return blazeEntity;
      });
      scene.idle(20);
      ...
      scene.world().showSection(util.select().position(1, 1, 2), Direction.DOWN);
      scene.idle(20);
      scene.world().modifyBlock(util.grid().at(1, 1, 2), s -> (BlockState)s.setValue(BlazeBurnerBlock.HEAT_LEVEL, BlazeBurnerBlock.HeatLevel.KINDLED), false);
      scene.overlay()
         .showText(70)
      ...
      scene.idle(20);
      scene.overlay().showControls(util.vector().topOf(center.east().above()), Pointing.DOWN, 40).rightClick().withItem(new ItemStack(Items.FLINT_AND_STEEL));
      scene.idle(7);
      ...
      scene.overlay()
         ...
         .placeNearTarget();
      scene.idle(80);
      scene.overlay().showControls(util.vector().topOf(center.east().above()), Pointing.DOWN, 40).rightClick().withItem(new ItemStack(Items.SOUL_SAND));
      scene.idle(7);
      scene.world()
         .modifyBlock(util.grid().at(3, 1, 2), s -> (BlockState)s.setValue(LitBlazeBurnerBlock.FLAME_TYPE, LitBlazeBurnerBlock.FlameType.SOUL), false);
      scene.overlay()
         .showText(60)
   ...
   public static void blazeBurner(SceneBuilder scene, SceneBuildingUtil util) {
      ...
      scene.idle(7);
      scene.world().modifyBlock(burner, s -> (BlockState)s.setValue(BlazeBurnerBlock.HEAT_LEVEL, BlazeBurnerBlock.HeatLevel.KINDLED), false);
      scene.idle(20);
      scene.overlay()
         .showText(70)
         .attachKeyFrame()
      ...
      scene.idle(7);
      scene.world().modifyBlock(burner, s -> (BlockState)s.setValue(BlazeBurnerBlock.HEAT_LEVEL, BlazeBurnerBlock.HeatLevel.SEETHING), false);
      scene.idle(20);
      scene.overlay()
         .showText(80)
         .attachKeyFrame()
   ...
   public static void basin(SceneBuilder builder, SceneBuildingUtil util) {
      ...
      scene.world().modifyBlock(basinPos, s -> (BlockState)s.setValue(BasinBlock.FACING, Direction.DOWN), false);
      ...
      scene.idle(10);
      scene.world().modifyBlock(basinPos, s -> (BlockState)s.setValue(BasinBlock.FACING, Direction.NORTH), false);
      scene.idle(10);
      scene.overlay()
         .showText(80)
         .attachKeyFrame()
         ...
         .pointAt(basinSide.add(0.15, 0.0, -0.5))
         .placeNearTarget();
      scene.idle(90);
      ...
      scene.world()
         .modifyBlockEntityNBT(
            ...
            nbt -> nbt.put(
                  "VisualizedItems",
                  NBTHelper.writeCompoundList(
                     ImmutableList.of(IntAttached.with(1, new ItemStack(Blocks.BRICKS))),
                     ia -> (CompoundTag)((ItemStack)ia.getValue()).saveOptional(scene.world().getHolderLookupProvider())
                  )
               )
         );
      ...
      scene.overlay()
         ...
         .pointAt(basinSide.add(0.0, -1.0, -1.0))
         .placeNearTarget();
      scene.idle(70);
      scene.world().hideIndependentSection(depot, Direction.NORTH);
      scene.idle(10);
      ...
      scene.overlay()
         ...
         .pointAt(util.vector().topOf(util.grid().at(3, 1, 2)).subtract(0.0, 0.1875, 0.0))
         .placeNearTarget();
      scene.idle(80);
