...
public class DetectorScenes {
   ...
   public static void thresholdSwitch(SceneBuilder builder, SceneBuildingUtil util) {
      ...
      scene.overlay()
         ...
         .pointAt(upper.subtract(0.125, 0.0, 0.0))
         .placeNearTarget();
      scene.idle(60);
      scene.world().removeItemsFromBelt(util.grid().at(3, 0, 2));
      scene.world().flapFunnel(util.grid().at(3, 1, 2), false);
      ...
      for (int ix = 0; ix < 5; ix++) {
         ...
         if (ix % 3 == 1) {
            scene.world()
               .modifyBlock(switchPos, s -> (BlockState)s.setValue(ThresholdSwitchBlock.LEVEL, (Integer)s.getValue(ThresholdSwitchBlock.LEVEL) - 1), false);
         }
      }
      ...
      scene.overlay()
         ...
         .pointAt(lower.subtract(0.125, 0.0, 0.0))
         .placeNearTarget();
      scene.idle(30);
      ...
      scene.world().modifyBlock(switchPos, s -> (BlockState)s.setValue(ThresholdSwitchBlock.LEVEL, 0), false);
      scene.idle(5);
