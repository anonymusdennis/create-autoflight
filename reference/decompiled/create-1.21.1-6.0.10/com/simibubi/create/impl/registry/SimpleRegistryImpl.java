...
public abstract sealed class SimpleRegistryImpl<K, V> implements SimpleRegistry<K, V> permits SimpleRegistryImpl.SingleImpl, SimpleRegistryImpl.MultiImpl {
   ...
   public synchronized void register(K object, V value) {
      ...
      if (existing != null) {
         throw new IllegalArgumentException(
            String.format("Tried to register duplicate values for object %s (%s): old=%s, new=%s", object, object.getClass(), existing, value)
         );
      } else {
   ...
   public synchronized V get(StateHolder<K, ?> state) {
      ...
      K owner = (K)((StateHolderAccessor)state).getOwner();
      return this.get(owner);
   }

   public static <K, V> SimpleRegistry<K, V> single() {
      return new SimpleRegistryImpl.SingleImpl<>();
   }

   public static <K, V> SimpleRegistry.Multi<K, V> multi() {
      return new SimpleRegistryImpl.MultiImpl<>();
   }
   ...
   static final class MultiImpl<K, V> extends SimpleRegistryImpl<K, List<V>> implements SimpleRegistry.Multi<K, V> {
      ...
      public void addProvider(SimpleRegistry.Provider<K, V> provider) {
         this.registerProvider(new SimpleRegistryImpl.MultiImpl.ProviderWrapper<>(provider));
      }
      ...
      private List<V> calculateTotal(K object) {
         ...
         for (SimpleRegistry.Provider<K, List<V>> provider : this.providers) {
      ...
      public synchronized List<V> get(StateHolder<K, ?> state) {
         return (List<V>)super.get(state);
      }

      private static record ProviderWrapper<K, V>(SimpleRegistry.Provider<K, V> wrapped) implements SimpleRegistry.Provider<K, List<V>> {
   ...
   static final class SingleImpl<K, V> extends SimpleRegistryImpl<K, V> {
      ...
      public synchronized V get(K object) {
         ...
         if (this.registrations.containsKey(object)) {
            ...
            for (SimpleRegistry.Provider<K, V> provider : this.providers) {
