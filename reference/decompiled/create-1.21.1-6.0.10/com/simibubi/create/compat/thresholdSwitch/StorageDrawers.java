...
public class StorageDrawers implements ThresholdSwitchCompat {
   ...
   public boolean isFromThisMod(BlockEntity blockEntity) {
      return blockEntity != null && Mods.STORAGEDRAWERS.id().equals(RegisteredObjectsHelper.getKeyOrThrow(blockEntity.getType()).getNamespace());
   }

   @Override
   public long getSpaceInSlot(IItemHandler inv, int slot) {
      return slot == 0 ? 0L : (long)inv.getSlotLimit(slot);
   }
}
