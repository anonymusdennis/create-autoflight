...
public class CrushingCategory extends CreateRecipeCategory<AbstractCrushingRecipe> {
   ...
   public void setRecipe(IRecipeLayoutBuilder builder, AbstractCrushingRecipe recipe, IFocusGroup focuses) {
      builder.addSlot(RecipeIngredientRole.INPUT, 51, 3).setBackground(getRenderedSlot(), -1, -1).addIngredients((Ingredient)recipe.getIngredients().get(0));
      int xOffset = this.getBackground().getWidth() / 2;
      int yOffset = 86;
      this.layoutOutput(recipe)
         .forEach(
            layoutEntry -> ((IRecipeSlotBuilder)builder.addSlot(RecipeIngredientRole.OUTPUT, xOffset + layoutEntry.posX() + 1, yOffset + layoutEntry.posY() + 1)
   ...
   private List<CrushingCategory.LayoutEntry> layoutOutput(StandardProcessingRecipe<?> recipe) {
