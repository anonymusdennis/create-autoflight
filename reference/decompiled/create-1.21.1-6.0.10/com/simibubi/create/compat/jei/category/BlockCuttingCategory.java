...
public class BlockCuttingCategory extends CreateRecipeCategory<BlockCuttingCategory.CondensedBlockCuttingRecipe> {
   ...
   public void setRecipe(IRecipeLayoutBuilder builder, BlockCuttingCategory.CondensedBlockCuttingRecipe recipe, IFocusGroup focuses) {
      ...
      builder.addSlot(RecipeIngredientRole.INPUT, 5, 5)
         ...
         .addItemStacks(Arrays.asList(((Ingredient)recipe.getIngredients().get(0)).getItems()));
      int i = 0;
   ...
   public void draw(
      BlockCuttingCategory.CondensedBlockCuttingRecipe recipe, IRecipeSlotsView recipeSlotsView, GuiGraphics graphics, double mouseX, double mouseY
   ) {
   ...
   public static List<RecipeHolder<BlockCuttingCategory.CondensedBlockCuttingRecipe>> condenseRecipes(List<RecipeHolder<?>> stoneCuttingRecipes) {
      List<RecipeHolder<BlockCuttingCategory.CondensedBlockCuttingRecipe>> condensed = new ArrayList<>();

      label23:
      for (RecipeHolder<?> recipe : stoneCuttingRecipes) {
         Ingredient i1 = (Ingredient)recipe.value().getIngredients().get(0);

         for (RecipeHolder<BlockCuttingCategory.CondensedBlockCuttingRecipe> condensedRecipe : condensed) {
            if (ItemHelper.matchIngredients(i1, (Ingredient)((BlockCuttingCategory.CondensedBlockCuttingRecipe)condensedRecipe.value()).getIngredients().get(0))
               )
             {
               ((BlockCuttingCategory.CondensedBlockCuttingRecipe)condensedRecipe.value()).addOutput(getResultItem(recipe.value()));
               continue label23;
            }
         }
