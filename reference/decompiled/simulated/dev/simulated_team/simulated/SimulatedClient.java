...
public class SimulatedClient {
   ...
   public static void init() {
      ...
      VeilEventPlatform.INSTANCE.onVeilAddShaderProcessors((provider, registry) -> registry.addPreprocessor(new EndSeaFadeTransformer(), false));
      VeilEventPlatform.INSTANCE.onVeilRegisterFixedBuffers(registry -> {
         registry.registerFixedBuffer(Stage.AFTER_PARTICLES, SimRenderTypes.laser());
         registry.registerFixedBuffer(Stage.AFTER_TRANSLUCENT_BLOCKS, SimRenderTypes.lens());
         registry.registerFixedBuffer(Stage.AFTER_LEVEL, SimRenderTypes.staffOverlay());
      });
