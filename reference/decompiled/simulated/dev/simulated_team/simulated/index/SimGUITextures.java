...
public enum SimGUITextures implements ScreenElement {
   ...
   private SimGUITextures(final String location, final int width, final int height) {
      this(location, 0, 0, width, height);
   }

   private SimGUITextures(final int startX, final int startY) {
      this("icons", startX * 16, startY * 16, 16, 16);
   }

   private SimGUITextures(final String location, final int startX, final int startY, final int width, final int height) {
      this("simulated", location, startX, startY, width, height);
   }

   private SimGUITextures(final String location, final int startX, final int startY, final int width, final int height, final int texWidth, final int texHeight) {
      this("simulated", location, startX, startY, width, height, texWidth, texHeight);
   }

   private SimGUITextures(final String namespace, final String location, final int startX, final int startY, final int width, final int height) {
      this(namespace, location, startX, startY, width, height, 256, 256);
   }

   private SimGUITextures(
      final ResourceLocation location, final int startX, final int startY, final int width, final int height, final int texWidth, final int texHeight
   ) {
   ...
   private SimGUITextures(
      ...
      ResourceLocation loc = ResourceLocation.tryBuild(namespace, "textures/gui/" + location + ".png");
   ...
   public void render(GuiGraphics graphics, int x, int y, Color c) {
      this.bind();
      UIRenderHelper.drawColoredTexture(graphics, c, x, y, 0, (float)this.startX, (float)this.startY, this.width, this.height, this.texWidth, this.texHeight);
   }
}
