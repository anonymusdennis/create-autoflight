...
public class SimWorldPresets {
   ...
   public static final SimulatedWorldPreset AIRSHIP_READY = create(
      AirshipReadyPreset::new, "airship_ready", Component.translatable("generator.simulated.airship_ready.info")
   );
   public static final SimulatedWorldPreset END_SEA = create(EndSeaPreset::new, "end_sea", null);

   private static <T extends SimulatedWorldPreset> T create(
      BiFunction<ResourceLocation, Component, T> constructor, String name, @Nullable Component description
   ) {
      ResourceLocation id = Simulated.path(name);
      T preset = (T)constructor.apply(id, description);
