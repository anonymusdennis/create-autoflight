...
public record BlockFrictionItemAttribute(double friction) implements ItemAttribute {
   ...
   public boolean appliesTo(ItemStack stack, Level world) {
      if (stack.getItem() instanceof BlockItem item) {
         ...
         return (Double)extension.sable$getProperty((PhysicsBlockPropertyType)PhysicsBlockPropertyTypes.FRICTION.get()) == this.friction();
      } else {
         return false;
      }
   }
   ...
   public static class Type implements ItemAttributeType {
      ...
      public List<ItemAttribute> getAllAttributes(ItemStack stack, Level level) {
         if (stack.getItem() instanceof BlockItem item) {
            BlockStateExtension extension = (BlockStateExtension)item.getBlock().defaultBlockState();
            double mass = (Double)extension.sable$getProperty((PhysicsBlockPropertyType)PhysicsBlockPropertyTypes.FRICTION.get());
      ...
      public MapCodec<? extends ItemAttribute> codec() {
         return BlockFrictionItemAttribute.CODEC;
      }

      public StreamCodec<? super RegistryFriendlyByteBuf, ? extends ItemAttribute> streamCodec() {
         return BlockFrictionItemAttribute.STREAM_CODEC;
      }
   }
}
