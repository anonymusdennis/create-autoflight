...
public class ErrorBuilder {
   ...
   private static String addPaddingToLine(int maxMargin, ErrorLine errorLine) {
      ...
      return neededMargin >= 0 ? StringUtil.repeatChar(' ', maxMargin - neededMargin) + errorLine.build() : errorLine.build();
   }
   ...
   public void nested(ErrorBuilder err) {
      err.getLineStream().map(NestedLine::new).forEach(this.lines::add);
   }
}
