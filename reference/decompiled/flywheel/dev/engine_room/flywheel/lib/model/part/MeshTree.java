...
public final class MeshTree {
   private static final ThreadLocal<MeshTree.ThreadLocalObjects> THREAD_LOCAL_OBJECTS = ThreadLocal.withInitial(MeshTree.ThreadLocalObjects::new);
   private static final Pose IDENTITY_POSE = new PoseStack().last();
   ...
   private static MeshTree convert(ModelPart modelPart, MeshTree.ThreadLocalObjects objects) {
   ...
   @Nullable
   private static Mesh compile(ModelPart modelPart, MeshTree.ThreadLocalObjects objects) {
      if (modelPart.isEmpty()) {
         return null;
      } else {
         VertexWriter vertexWriter = objects.vertexWriter;
         FlwLibLink.INSTANCE.compileModelPart(modelPart, IDENTITY_POSE, vertexWriter, 15728880, OverlayTexture.NO_OVERLAY, -1);
