...
public class AeronauticsClient {
   ...
   private static void registerEvents() {
      VeilEventPlatform.INSTANCE
         .onVeilRenderLevelStage(
            (stage, levelRenderer, bufferSource, matrixStack, frustumMatrix, projectionMatrix, renderTick, deltaTracker, camera, frustum) -> ClientBalloonEffectRenderer.onRenderLevelStage(
                  stage, frustumMatrix, projectionMatrix, renderTick
               )
         );
      ...
      VeilEventPlatform.INSTANCE.onVeilRegisterFixedBuffers(registry -> {
         registry.registerFixedBuffer(Stage.AFTER_BLOCK_ENTITIES, AeroRenderTypes.levitite());
         registry.registerFixedBuffer(Stage.AFTER_WEATHER, AeroRenderTypes.levititeGhosts());
      });
   }
}
