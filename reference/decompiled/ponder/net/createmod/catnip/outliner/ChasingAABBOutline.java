...
public class ChasingAABBOutline extends AABBOutline {
   ...
   private static AABB interpolateBBs(AABB current, AABB target, float pt) {
      return new AABB(
         Mth.lerp((double)pt, current.minX, target.minX),
         Mth.lerp((double)pt, current.minY, target.minY),
         Mth.lerp((double)pt, current.minZ, target.minZ),
         Mth.lerp((double)pt, current.maxX, target.maxX),
         Mth.lerp((double)pt, current.maxY, target.maxY),
         Mth.lerp((double)pt, current.maxZ, target.maxZ)
      );
   }
}
