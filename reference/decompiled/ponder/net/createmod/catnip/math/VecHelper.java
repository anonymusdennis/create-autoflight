...
public class VecHelper {
   ...
   public static Vec3 offsetRandomly(Vec3 vec, RandomSource r, float radius) {
      return new Vec3(
         vec.x + (double)((r.nextFloat() - 0.5F) * 2.0F * radius),
         vec.y + (double)((r.nextFloat() - 0.5F) * 2.0F * radius),
         vec.z + (double)((r.nextFloat() - 0.5F) * 2.0F * radius)
      );
   }
   ...
   public static Vec3 slerp(float p, Vec3 from, Vec3 to) {
      ...
      return from.scale((double)Mth.sin(1.0F - p) * theta)
         .add(to.scale((double)Mth.sin((float)(theta * (double)p))))
         .scale((double)(1.0F / Mth.sin((float)theta)));
   }

   public static Vec3 clampComponentWise(Vec3 vec, float maxLength) {
      return new Vec3(
         Mth.clamp(vec.x, (double)(-maxLength), (double)maxLength),
         Mth.clamp(vec.y, (double)(-maxLength), (double)maxLength),
         Mth.clamp(vec.z, (double)(-maxLength), (double)maxLength)
      );
   }
   ...
   public static Vec3 projectToPlayerView(Vec3 target, float partialTicks) {
      ...
      if ((Boolean)mc.options.bobView().get() && mc.getCameraEntity() instanceof Player playerEntity) {
         ...
         Quaternionf q2 = com.mojang.math.Axis.XP.rotationDegrees(Math.abs(Mth.cos(f1 * (float) Math.PI - 0.2F) * f2) * 5.0F);
         q2.conjugate();
         result3f.rotate(q2);
   ...
   public static Vec3 bezierDerivative(Vec3 p1, Vec3 p2, Vec3 q1, Vec3 q2, float t) {
      return p1.scale((double)(-3.0F * t * t + 6.0F * t - 3.0F))
         .add(q1.scale((double)(9.0F * t * t - 12.0F * t + 3.0F)))
         .add(q2.scale((double)(-9.0F * t * t + 6.0F * t)))
         .add(p2.scale((double)(3.0F * t * t)));
   }
   ...
   public static double alignedDistanceToFace(Vec3 pos, BlockPos blockPos, Direction face) {
      ...
      return (double)Math.abs(getCoordinate(pos, axis) - (float)(blockPos.get(axis) + (face.getAxisDirection() == AxisDirection.POSITIVE ? 1 : 0)));
   }
}
