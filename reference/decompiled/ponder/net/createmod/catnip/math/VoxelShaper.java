...
public class VoxelShaper {
   ...
   public static VoxelShaper forHorizontal(VoxelShape shape, Direction facing) {
      return forDirectionsWithRotation(shape, facing, Plane.HORIZONTAL, new VoxelShaper.HorizontalRotationValues());
   }
   ...
   public static VoxelShaper forAxis(VoxelShape shape, Axis along) {
      return forDirectionsWithRotation(
         shape, axisAsFace(along), Arrays.asList(Direction.SOUTH, Direction.EAST, Direction.UP), new VoxelShaper.DefaultRotationValues()
      );
   }
   ...
   protected static VoxelShaper forDirectionsWithRotation(
      VoxelShape shape, Direction facing, Iterable<Direction> directions, Function<Direction, Vec3> rotationValues
   ) {
   ...
   protected static VoxelShape rotate(VoxelShape shape, Direction from, Direction to, Function<Direction, Vec3> usingValues) {
      return from == to ? shape : rotatedCopy(shape, usingValues.apply(from).reverse().add(usingValues.apply(to)));
   }
   ...
   protected static VoxelShape rotatedCopy(VoxelShape shape, Vec3 rotation) {
      if (rotation.equals(Vec3.ZERO)) {
         ...
         shape.forAllBoxes((x1, y1, z1, x2, y2, z2) -> {
            ...
            v1 = VecHelper.rotate(v1, (double)((float)rotation.x), Axis.X);
            v1 = VecHelper.rotate(v1, (double)((float)rotation.y), Axis.Y);
            v1 = VecHelper.rotate(v1, (double)((float)rotation.z), Axis.Z).add(center);
            v2 = VecHelper.rotate(v2, (double)((float)rotation.x), Axis.X);
            v2 = VecHelper.rotate(v2, (double)((float)rotation.y), Axis.Y);
            v2 = VecHelper.rotate(v2, (double)((float)rotation.z), Axis.Z).add(center);
   ...
   protected static class DefaultRotationValues implements Function<Direction, Vec3> {
      public Vec3 apply(Direction direction) {
         return new Vec3(
            direction == Direction.UP ? 0.0 : (double)(Plane.VERTICAL.test(direction) ? 180 : 90),
            (double)(-VoxelShaper.horizontalAngleFromDirection(direction)),
            0.0
         );
      }
   }
