...
public class PonderLevel extends SchematicLevel {
   ...
   public void createBackup() {
      ...
      this.blockEntities.forEach((k, v) -> this.originalBlockEntities.put(k, v.saveWithFullMetadata(this.registryAccess())));
      this.entities.forEach(e -> {
         CompoundTag tag = new CompoundTag();
         e.save(tag);
         EntityType.create(tag, this).ifPresent(this.originalEntities::add);
      });
   }
   ...
   public void restore() {
      ...
      this.originalBlockEntities.forEach((k, v) -> {
         BlockEntity blockEntity = BlockEntity.loadStatic(k, this.originalBlocks.get(k), v, this.registryAccess());
      ...
      this.originalEntities.forEach(e -> {
         CompoundTag tag = new CompoundTag();
         e.save(tag);
         EntityType.create(tag, this).ifPresent(this.entities::add);
      });
   ...
   public void restoreBlocks(Selection selection) {
      selection.forEach(p -> {
         ...
         if (this.originalBlockEntities.containsKey(p)) {
            BlockEntity blockEntity = BlockEntity.loadStatic(p, this.originalBlocks.get(p), this.originalBlockEntities.get(p), this.registryAccess());
            if (blockEntity != null) {
               this.onBEAdded(blockEntity, blockEntity.getBlockPos());
   ...
   public BlockState getBlockState(BlockPos globalPos) {
      if (this.mask != null && !this.mask.test(globalPos.subtract(this.anchor))) {
         ...
         return this.currentlyTickingEntities && globalPos.getY() < 0 ? Blocks.AIR.defaultBlockState() : super.getBlockState(globalPos);
      }
   }
