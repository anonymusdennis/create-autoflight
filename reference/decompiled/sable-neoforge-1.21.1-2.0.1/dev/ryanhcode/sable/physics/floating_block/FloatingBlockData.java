...
public class FloatingBlockData {
   ...
   private void addData(Vector3dc pos, double scale) {
      ...
      pos.fma(-1.0 / this.totalScale, this.weightedPosition, tempPosOffset);
   ...
   public void computePressureScale(SubLevel subLevel) {
      subLevel.logicalPose().orientation().transform(this.weightedPosition, positionTemp);
      subLevel.logicalPose().position().fma(1.0 / this.totalScale, positionTemp, positionTemp);
      this.latestPressureScale = DimensionPhysicsData.getAirPressure(subLevel.getLevel(), positionTemp);
   }
}
